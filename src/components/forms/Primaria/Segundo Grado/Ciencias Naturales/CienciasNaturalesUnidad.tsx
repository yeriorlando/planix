...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Ofrece explicaciones científicas y tecnológicas e inferencias de observaciones, exploraciones guiadas y cuestionamientos de fenómenos naturales.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Aplica procedimientos organizados, sistemáticos y creativos explorando, construyendo, simulando y haciéndose consciente de sus cuestionamientos a partir de observación y medición.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Desarrollo Personal y Espiritual; Ambiental y de la Salud",
    defaultDesc:
      "Asume una actitud constructiva, reflexiva y en armonía en sí mismo, con los demás y su entorno, tomando acciones proactivas en atención a su bienestar.",
  },
];
...
const FALLBACK_INDICATORS = [
  ...
  "**Identifica y describe propiedades** utilizando el lenguaje apropiado que caracterizan los estados del agua y a las mezclas, los cambios en la materia, ciclo del agua.",
...
const DRAFT_KEY = "plx:naturales2do_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias de la Naturaleza de 2do. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('naturales', '2do');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'naturales' && cu.grade_id === '2do')
        .map(cu => cu.content);
