...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Compara conceptos e informaciones de su historia familiar y comunitaria, con la finalidad de reflexionar en forma crítica sobre sus ideas y soluciones creativas a situaciones diversas, utilizando la tecnología.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Comprende en forma crítica conceptos y situaciones problemáticas del espacio natural y social de su entorno comunitario, con la finalidad de aportar ideas a las soluciones creativas utilizando tecnología.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Desarrollo Personal y Espiritual; Ambiental y de la Salud",
    defaultDesc:
      "Emplea actitudes de respeto a sí mismo y a las demás personas en cualquier espacio; con la finalidad de construir una ciudadanía basada en la participación democrática, la exigencia de sus derechos, el cumplimiento de sus deberes y el cuidado de su entorno natural y social.",
  },
];
...
const FALLBACK_INDICATORS = [
  "**Muestra compresión de informaciones** de procesos históricos y geográficos de su entorno social y natural, a través de narraciones históricas sobre estos.",
  ...
  "**Se involucra en indagaciones** de solución creativa a situaciones problemáticas que afectan su entorno familiar y comunitario, utilizando la tecnología.",
...
const DRAFT_KEY = "plx:sociales2do_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          ...
          (p.grado?.toLowerCase().includes("2") || p.grado?.toLowerCase().includes("segund"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias Sociales de 2do. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('sociales', '2do');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'sociales' && cu.grade_id === '2do')
        .map(cu => cu.content);
