...
const DRAFT_KEY = "plx:educacionfisica2do_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          (p.asignatura?.toLowerCase().includes("educacion-fisica") || p.titulo?.toLowerCase().includes("educacion-fisica")) &&
          (p.grado?.toLowerCase().includes("2") || p.grado?.toLowerCase().includes("segundo"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Física de 2do. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-fisica', '2do');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-fisica' && cu.grade_id === '2do')
        .map(cu => cu.content);
