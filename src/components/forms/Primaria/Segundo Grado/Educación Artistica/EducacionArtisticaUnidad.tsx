...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Usa imágenes, movimientos, textos, gestos, colores y materiales diversos, para comunicar ideas, emociones, sentimientos y vivencias, en contextos diversos.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Incorpora en sus expresiones artísticas, características de personajes, instrumentos, objetos y manifestaciones de su comunidad; con la finalidad de usarlos como elementos de comunicación de ideas, sentimientos y vivencias en la solución de problemas de manera creativa en distintos contextos.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Desarrollo Personal y Espiritual; Ambiental y de la Salud",
    defaultDesc:
      "Integra ideas y situaciones de su entorno natural y cultural en sus creaciones; con el objeto de reafirmar su identidad personal y social, así como el interés por su salud y el medio ambiente.",
  },
];
...
const FALLBACK_INDICATORS = [
  "**Interpreta y construye** mensajes comunicándolos a través de imágenes, textos, gestos, movimientos, sonidos y colores.",
  ...
  "**Utiliza adecuadamente elementos** de los lenguajes artísticos y recursos tecnológicos al difundir ideas, situaciones, temas y problemáticas relevantes.",
...
const DRAFT_KEY = "plx:artistica2do_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Artística de 2do. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-artistica', '2do');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-artistica' && cu.grade_id === '2do')
        .map(cu => cu.content);
