...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Comprende y expresa ideas, sentimientos y hechos de forma oral y escrita en la lengua extranjera (inglés), utilizando recursos diversos para comunicarse de manera básica y efectiva en situaciones cotidianas de su entorno escolar y familiar.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Utiliza estrategias básicas de pensamiento lógico, creatividad y herramientas tecnológicas para resolver problemas comunicativos sencillos y organizar información relevante en la lengua extranjera (inglés).",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Desarrollo Personal y Espiritual; Ambiental y de la Salud",
    defaultDesc:
      "Demuestra respeto y valoración por la diversidad cultural y lingüística de las sociedades de habla inglesa, promoviendo relaciones pacíficas, armoniosas y valores ciudadanos a través de la comunicación.",
  },
];
...
const FALLBACK_INDICATORS = [
  "**Comprende y responde** a instrucciones, preguntas y saludos cotidianos sencillos de forma oral en inglés.",
  "**Identifica y asocia** palabras, expresiones y frases sencillas en inglés con las imágenes o gestos correspondientes.",
  "**Produce palabras, frases** y oraciones breves y sencillas en inglés para presentarse, saludar y expresar necesidades básicas.",
  "**Reconoce y escribe** palabras y frases de uso muy frecuente en inglés, apoyándose en imágenes y material visual.",
  "**Demuestra una actitud** de respeto, curiosidad y valoración positiva hacia el idioma inglés y las culturas de habla inglesa.",
  "**Utiliza recursos tecnológicos** y audiovisuales básicos para apoyar y mejorar su aprendizaje del idioma inglés.",
  "**Participa activamente en** juegos, canciones y dinámicas grupales sencillas en inglés, cooperando con sus compañeros."
];

export interface InglesUnidad4toProps {
  user: any;
...
const DRAFT_KEY = "plx:ingles4to_unidad_draft";
...
export default function InglesUnidad4to({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  const handlePreview = () => {
    ...
    const previewData = {
      ...
      titulo: `Unidad: ${selectedSequence?.name || "Lenguas Extranjeras - Inglés"}`,
  ...
  const handleGenerateContent = async () => {
    ...
    try {
      ...
      if (plan) {
        ...
        toast.success("¡Planificación de Inglés generada!");
      }
    } catch (err) {
      console.error(err);
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (inglesPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Inglés de 4to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('ingles', '4to');
      const filteredCustom = customUnitsRaw
        .filter(cu => (cu.subject_id === 'ingles' || cu.subject_id === 'ingles-pri') && cu.grade_id === '4to')
        .map(cu => cu.content);
      ...
      <div className="mx-auto max-w-5xl px-2 pb-8 pt-14 md:pt-20 text-left relative z-10">
        ...
        >
          ...
          <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            <Field label="Área">
              <input className={inputCls} value="Lenguas Extranjeras - Inglés" readOnly />
            </Field>
            <Field label="Unidad">
