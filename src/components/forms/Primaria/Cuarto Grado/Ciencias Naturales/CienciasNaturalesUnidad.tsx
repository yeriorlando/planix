...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Ofrece explicaciones científicas y tecnológicas a partir de analizar observaciones, medición, modelos y experimentación de fenómenos naturales fundamentales en contexto próximo o experimentado o modelado en ciencias de la vida, físicas, de la tierra y el universo.",
  },
  {
    key: "logico",
  ...
  },
  {
    ...
    defaultDesc:
      "Asume una actitud preventiva, creativa, curiosa, colaborativa, responsable y en armonía integral en sí mismo, con los demás, con su entorno y como parte de los seres vivos, tomando acciones básicas y proactivas en atención a su bienestar y uso sostenible de los recursos.",
  },
];
...
const ASIGNATURAS_PRIMARIA = [
  ...
  { name: "Ciencias de la Naturaleza", icon: Globe },
...
const FALLBACK_INDICATORS = [
  "**Describe o analiza características**, en los seres vivos de su entorno, células vegetales y animales, sus similitudes y diferencias vinculando con la biodiversidad de los seres vivos; funciones de los órganos en plantas y animales; los cambios que se presentan en los seres humanos y plantas; y el flujo de energía en el ecosistema y algunos métodos de siembra, tipos de cultivos, plagas y malezas.",
  "**Describe o clasifica propiedades**, que caracterizan sustancias, combinaciones y distintos tipos de mezclas; fenómenos simples comunes en los que ocurren: reflexión, refracción y difracción de la luz; la composición física del planeta Tierra, mostrando sus diferentes capas, mediante esquemas o maquetas; los sismos y el movimiento de la corteza terrestre.",
  "**Usa diferentes vías** de comunicación para explicar o dar soluciones de forma abierta y creativa a su manera de percibir las propiedades y características de la materia, propiedades de la luz, estructuras y mecanismo de máquinas, respetando las ideas y diferencias con las demás personas.",
  "**Ejecuta experimentos guiados** en colaboración; observando, describiendo, registrando y utilizando herramientas o equipos y siguiendo características y propiedades observables de seres vivos o ecosistema como las células vegetales y animales, así como también de objetos; de fenómenos naturales; de la luz; de las sustancias, combinaciones y las mezclas; de distintos movimientos de la Tierra y la Luna alrededor del Sol, incluyendo los eclipses; de la energía solar y los seres vivos; y de la obtención de fruto comestible.",
  "**Asocia y construye modelos**, estructuras y funciones de máquinas y mecanismo de transmisión de movimiento, objetos o juguetes, herramientas, sistema solar y tecnología de la comunicación de forma adecuada.",
  "**Comunica sus ideas** e inferencias de las observaciones, exploraciones y experimentos, usando y cuidando los sentidos e instrumentos para percibir, recolectar y obtener datos e información de su entorno; mostrando y argumentando los resultados de su trabajo de forma organizada y creativa en proyecto individual y colectivo.",
  "**Propone y argumenta medidas** de prevención de enfermedades tropicales virales, transmitidas por vectores que afectan a los seres humanos; de algunas sustancias tóxicas y radiaciones como rayos UV y rayos X.",
  "**Analiza y argumenta medidas** sobre el uso sostenible de los recursos naturales, el cuidado del Medioambiente, y los diferentes problemas ambientales que afectan el desarrollo sostenible en la región y el país.",
  "**Identifica y aplica actitudes** y valores proactivos en su desarrollo personal que incentiva como la imaginación, la curiosidad, la colaboración, la perseverancia, la innovación, la objetividad, la responsabilidad, la solidaridad, la creatividad, la adaptabilidad, la observación, interés por profesiones y actividades científicas y tecnológicas, y autogestionar sus emociones, preguntas y aprendizaje."
];

export interface CienciasNaturalesUnidadProps {
  user: any;
...
const DRAFT_KEY = "plx:naturales4to_draft";
...
export default function CienciasNaturalesUnidad({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  const handlePreview = () => {
    ...
    const previewData = {
      ...
      titulo: `Unidad: ${selectedSequence?.name || "Ciencias de la Naturaleza"}`,
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          (p.asignatura?.toLowerCase().includes("naturales") || p.titulo?.toLowerCase().includes("naturales")) &&
          (p.grado?.toLowerCase().includes("4") || p.grado?.toLowerCase().includes("cuarto"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias de la Naturaleza de 4to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('naturales', '4to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'naturales' && cu.grade_id === '4to')
        .map(cu => cu.content);
  ...
  return (
    <div className="w-full bg-transparent text-[#1B1B1B] dark:text-zinc-100 font-sans relative overflow-hidden">
      ...
      <DailyPlanSelector
        ...
        subjectId={selectedSubject?.id || "naturales"}
        grade={selectedGrade}
      />
