...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Expresa que su cuerpo cambia, se comunica y que necesita cuidado y protección, con la finalidad de reconocer su valor y el de los demás en diferentes contextos, tomando en cuenta que es criatura de Dios, hecho a su imagen y semejanza.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Distingue los aportes del trabajo humano y de las ciencias y tecnologías en la familia, la escuela y la comunidad, con el fin de reconocer en estas contribuciones la expresión del quehacer de Dios, con respeto, criticidad y asertividad.",
  },
  {
    key: "etica",
...
const DRAFT_KEY = "plx:formacion4to_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Formación Integral Humana y Religiosa de 4to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('formacion-humana', '4to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'formacion-humana' && cu.grade_id === '4to')
        .map(cu => cu.content);
