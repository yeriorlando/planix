...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Utiliza fuentes de información seguras sobre elementos físicos y políticos de la Geografía del Caribe y las Antillas; con la finalidad de expresar los vínculos que unen la región de América.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Crítico y Creativo; Resolución de Problemas; Científica y Tecnológica",
    defaultDesc:
      "Identifica problema en hechos históricos de los siglos XVI, XVII, XVIII y XIX; con el propósito de relacionarlos en forma lógica, creativa y crítica con el espacio geográfico en el que ocurrieron, expresando su opinión.",
  },
  {
    key: "etica",
...
const FALLBACK_INDICATORS = [
  ...
  "**Muestra dominio de** la organización lógica de problemas históricos de periodos identificados. Demuestra comprensión de la relación de los hechos históricos con el espacio geográfico en el ocurrieron.",
...
const DRAFT_KEY = "plx:sociales4to_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias Sociales de 4to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('sociales', '4to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'sociales' && cu.grade_id === '4to')
        .map(cu => cu.content);
