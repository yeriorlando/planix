...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Utiliza símbolos creados a partir del sonido, el gesto, el movimiento y la imagen, con el fin de integrarlos como elementos comunicativos de los lenguajes artísticos.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Analiza críticamente distintas manifestaciones artísticas en medios físicos y virtuales al crear o disfrutarlas o al describir elementos constitutivos presentes en ellas, a fin de resolver problemáticas de su contexto.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Desarrollo Personal y Espiritual; Ambiental y de la Salud",
    defaultDesc:
      "Plasma creativamente formas y temas inspirados en su contexto, haciendo uso adecuado de la voz, el cuerpo, recursos y técnicas; al comunicar sus ideas y sentimientos de dominicanidad.",
  },
];
...
const FALLBACK_INDICATORS = [
  ...
  "**Transforma creativamente elementos** de los lenguajes artísticos, desarrollando el pensamiento divergente.",
  "**Interactúa con naturalidad** al expresarse artísticamente, cuidando su cuerpo y su entorno, así como valorando capacidades, limitaciones y el esfuerzo personal.",
...
const DRAFT_KEY = "plx:artistica4to_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Artística de 4to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-artistica', '4to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-artistica' && cu.grade_id === '4to')
        .map(cu => cu.content);
