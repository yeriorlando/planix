...
const MATEMATICA_4TO_SEQUENCES = Object.values(compiled2ndCycleSequences as Record<string, any>)
  .filter((s: any) => s.subject_id === 'matematica-4to')
  ...
  .map((seq: any, idx: number) => {
    ...
    return {
      ...
      description: seq.description || `Secuencia didáctica de Matemática 4to grado.`,
...
export interface Matematica4toProps {
  user: any;
...
const DRAFT_KEY = "plx:matematica4to_draft";
...
export default function Matematica4to({
  user,
  selectedSequence,
  ...
  const getSequenceData = (idx: number) => {
    const original = MATEMATICA_4TO_SEQUENCES[idx] || MATEMATICA_4TO_SEQUENCES[0];
    ...
    try {
      ...
      if (!latestPlan) {
        latestPlan = {
          id: "temp-plan-id",
          docente_id: user.id,
          titulo: (activeActivityData as any)?.title || (activeActivityData as any)?.activityTitle || activeSequenceData?.title || "Matemática 4to Grado",
