...
const DRAFT_KEY = "plx:educacionfisica4to_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          ...
          (p.grado?.toLowerCase().includes("4") || p.grado?.toLowerCase().includes("cuart"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Física de 4to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-fisica', '4to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-fisica' && cu.grade_id === '4to')
        .map(cu => cu.content);
