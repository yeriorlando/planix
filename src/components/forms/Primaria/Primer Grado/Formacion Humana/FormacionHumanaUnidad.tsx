...
const DRAFT_KEY = "plx:formacion1ro_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Formación Integral Humana y Religiosa de 1ro. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('formacion-humana', '1ro');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'formacion-humana' && cu.grade_id === '1ro')
        .map(cu => cu.content);
