...
const DRAFT_KEY = "plx:educacionfisica1ro_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Física de 1ro. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-fisica', '1ro');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-fisica' && cu.grade_id === '1ro')
        .map(cu => cu.content);
