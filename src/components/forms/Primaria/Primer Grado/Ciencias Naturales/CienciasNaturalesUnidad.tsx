...
const FALLBACK_INDICATORS = [
  "**Comunica sus ideas**, observaciones, exploraciones y experimentos, usando y cuidando los sentidos e instrumentos para percibir información de su entorno; mostrando y argumentando los resultados de su trabajo de forma organizada y creativa en proyecto individual y colectivo.",
  ...
  "**Asocia y construye modelos**, estructuras y funciones de máquinas simples, objetos, herramientas, modelos arquitectónicos y tecnología de la comunicación de forma adecuada.",
  ...
  "**Identifica actitudes y valores** proactivos en su desarrollo personal que incentive como la curiosidad, la colaboración, la perseverancia, la solidaridad, creatividad, observation y auto gestionar sus emociones y preguntas."
];
...
const DRAFT_KEY = "plx:naturales1ro_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias de la Naturaleza de 1ro. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('naturales', '1ro');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'naturales' && cu.grade_id === '1ro')
        .map(cu => cu.content);
