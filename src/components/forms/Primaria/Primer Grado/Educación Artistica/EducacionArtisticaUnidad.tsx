...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Expresa artísticamente sentimientos y emociones producto de la percepción de sí mismo/a y de su entorno.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Utiliza en forma lógica, crítica y creativa elementos artísticos diversos en sus expresiones, identificándolos consigo mismo y sus contextos.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Desarrollo Personal y Espiritual; Ambiental y de la Salud",
    defaultDesc:
      "Representa situaciones de su entorno explorando las capacidades expresivas de su cuerpo; con el fin de mostrar respeto a sí mismo(a), a otras personas y del entorno natural y social, en interacción con estos.",
  },
];
...
const ASIGNATURAS_PRIMARIA = [
  ...
  { name: "Educación Artística", icon: Globe },
...
const DRAFT_KEY = "plx:artistica1ro_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          (p.asignatura?.toLowerCase().includes("educacion-artistica") || p.titulo?.toLowerCase().includes("educacion-artistica")) &&
          (p.grado?.toLowerCase().includes("1") || p.grado?.toLowerCase().includes("primer"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Artística de 1ro. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-artistica', '1ro');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-artistica' && cu.grade_id === '1ro')
        .map(cu => cu.content);
