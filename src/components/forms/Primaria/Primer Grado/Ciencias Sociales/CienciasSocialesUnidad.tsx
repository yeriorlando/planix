...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Identifica informaciones familiares, escolares y comunitarias; con la finalidad de conocer su entorno y relacionar el presente con el pasado.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Reconoce conceptos e informaciones básicas de su familia y comunidad; con la finalidad de reflexionar en forma crítica, comentando sus ideas y soluciones creativas a situaciones, apoyadas en la tecnología.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Desarrollo Personal y Espiritual; Ambiental y de la Salud",
    defaultDesc:
      "Muestra respeto a sí mismo y a las demás personas; con la finalidad de construir una ciudadanía basada en la participación democrática, la exigencia de sus derechos y el cumplimiento de sus deberes y cuidado de su entorno natural y social.",
  },
];
...
const FALLBACK_INDICATORS = [
  "**Identifica en el** mapa de la isla de Santo Domingo la división política de los países que la componen.",
  "**Utiliza conceptos de** orientación temporal y espacial para comunicar información acerca de su entorno social y natural.",
  "**Identifica la relación** entre los integrantes de su familia con su identidad personal.",
  "**Comprende información relacionadas** con situaciones familiares, comunitarias, los símbolos patrios, los héroes y las heroínas nacionales.",
  "**Analizar cambios en** el entorno natural y social de su familia y comunidad.",
  "**Cumple con sus** deberes y respeta los derechos en los contextos familiares, escolares y comunitarios.",
  "**Identifica prácticas y** hábitos saludables para el cuidado de su cuerpo y del entorno natural y social, y los aplica en su vida diaria.",
  "**Utiliza la creatividad** para identificar y resolver problemas en su entorno familiar y comunitario.",
  "**Participa activamente en** actividades que le preparan para ser un ciudadano democrático y responsable."
];

export interface CienciasSocialesUnidadProps {
  user: any;
...
const DRAFT_KEY = "plx:sociales1ro_draft";
...
export default function CienciasSocialesUnidad({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  const handlePreview = () => {
    ...
    const previewData = {
      ...
      titulo: `Unidad: ${selectedSequence?.name || "Ciencias Sociales"}`,
  ...
  const handleGenerateContent = async () => {
    ...
    try {
      ...
      if (plan) {
        ...
        toast.success("¡Planificación de Ciencias Sociales generada!");
      }
    } catch (err) {
      console.error(err);
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          ...
          (p.grado?.toLowerCase().includes("1") || p.grado?.toLowerCase().includes("primer"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias Sociales de 1ro. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('sociales', '1ro');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'sociales' && cu.grade_id === '1ro')
        .map(cu => cu.content);
      ...
      <div className="mx-auto max-w-5xl px-2 pb-8 pt-14 md:pt-20 text-left relative z-10">
        ...
        >
          ...
          <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            <Field label="Área">
              <input className={inputCls} value="Ciencias Sociales" readOnly />
            </Field>
            <Field label="Unidad">
        ...
        >
          {(() => {
            const ejeIcons: Record<string, React.ReactNode> = {
              "Educación Ambiental": <Leaf className="h-4 w-4" />,
              "Educación en Valores": <Heart className="h-4 w-4" />,
              "Educación Vial": <Car className="h-4 w-4" />,
              "Educación en Género": <Users className="h-4 w-4" />,
              "Educación para la Salud": <HeartPulse className="h-4 w-4" />,
              "Educación para la Paz": <Handshake className="h-4 w-4" />,
            };
            return (
