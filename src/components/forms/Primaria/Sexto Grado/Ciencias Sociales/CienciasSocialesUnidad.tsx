...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Comprueba la autoría de informaciones sobre la geografía, su clasificación e importancia; así como los aspectos geográficos e históricos de las civilizaciones antiguas y modernas, con la finalidad de producir valoraciones e informaciones científicas.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Crítico y Creativo; Resolución de Problemas; Científica y Tecnológica",
    defaultDesc:
      "Analiza acontecimientos históricos relacionados con las civilizaciones antiguas y modernas, la historia de América durante los siglos XVIII y XIX, situaciones políticas, sociales, económicas y culturales del mundo, especialmente y la República Dominicana, ocurridos durante la primera y segunda mitad del siglo XIX y el XX; con la finalidad de comprender las causas que dieron origen al problema y expresar su opinión en forma crítica y creativa, usando la tecnología.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Ambiental y de la Salud; Desarrollo Personal y Espiritual",
    defaultDesc:
      "Promueve en sus actuaciones sociales, relaciones democráticas, la conservación del patrimonio histórico, natural y cultural dominicano y del mundo; con la finalidad de construir una ciudadanía consciente de sus deberes y derechos, y de los derechos de las demás personas, promotora de la interculturalidad y defensora de la paz en su contexto cercano y en el mundo.",
  },
];
...
const FALLBACK_INDICATORS = [
  ...
  "**Maneja las reglas** básicas de respeto a la autoría en el uso de informaciones sobre el relieve, clima, hidrografía, economía, población y cultura de los continentes fuentes impresas y digitales.",
  ...
  "**Manifiesta en sus** relaciones actitudes democráticas en la consciencia de sus derechos y obligaciones con la defensa del patrimonio histórico, natural y cultural dominicano y del mundo.",
  "**Muestra convivencia armoniosa** con actitud receptiva a la cultura de personas de otra nacionalidad.",
  "**Demuestra sensibilidad ante** los problemas sociales, contribuyendo con sus ideas a la construcción de la cultura de paz y democrática en el País y en el mundo."
];

export interface CienciasSocialesUnidad6toProps {
  user: any;
...
const DRAFT_KEY = "plx:sociales6to_draft";
...
export default function CienciasSocialesUnidad6to({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  onCancel,
  onSave,
}: CienciasSocialesUnidad6toProps) {
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          ...
          (p.grado?.toLowerCase().includes("6") || p.grado?.toLowerCase().includes("sexto"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias Sociales de 6to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('sociales', '6to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'sociales' && cu.grade_id === '6to')
        .map(cu => cu.content);
