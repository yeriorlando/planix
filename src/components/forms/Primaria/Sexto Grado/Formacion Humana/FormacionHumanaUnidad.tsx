...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Promueve el valor de su cuerpo, el derecho de la niñez a ser protegida de todo tipo de maltrato y explotación a fin de promover su bienestar y el de los demás, con esmero, respeto y agradecimiento a Dios.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Promueve el valor del trabajo humano, el progreso de las ciencias y las tecnologías como expresión del amor de Dios y el desarrollo de los pueblos a fin de reconocer el esfuerzo, sacrificio de las personas que trabajan, con responsabilidad, respeto y asertividad.",
  },
  {
    key: "etica",
...
const FALLBACK_INDICATORS = [
  ...
  "**Da razones por** las que el trabajo humano es un medio de realización, satisfacción, así como de servicio personal y social para el desarrollo de los pueblos como expresión del amor de Dios.",
...
export interface FormacionHumanaUnidad6toProps {
  user: any;
...
const DRAFT_KEY = "plx:formacion6to_unidad_draft";
...
export default function FormacionHumanaUnidad6to({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Formación Integral Humana y Religiosa de 6to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('formacion-humana', '6to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'formacion-humana' && cu.grade_id === '6to')
        .map(cu => cu.content);
