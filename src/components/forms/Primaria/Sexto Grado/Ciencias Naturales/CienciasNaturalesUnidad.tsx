...
const FALLBACK_INDICATORS = [
  ...
  "**Aplica los principios** de Pascal y Arquímedes en máquinas, presenting sus modelos e ideas creativamente en colaboración con las demás personas.",
...
export interface CienciasNaturalesUnidad6toProps {
  user: any;
...
export default function CienciasNaturalesUnidad6to({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  onCancel,
  onSave,
}: CienciasNaturalesUnidad6toProps) {
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias de la Naturaleza de 6to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('naturales', '6to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'naturales' && cu.grade_id === '6to')
        .map(cu => cu.content);
