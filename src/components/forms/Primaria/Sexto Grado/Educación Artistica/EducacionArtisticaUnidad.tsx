...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Aplica diversas técnicas artísticas con la finalidad de comunicar ideas, sentimiento y vivencias, ejercitando la imaginación creadora.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Científica y Tecnológica",
    defaultDesc:
      "Realiza muestras, físicas o digitales, de creaciones artísticas y culturales, como resultado de sus investigaciones y de los medios y técnicas explorados, con la finalidad de comprender situaciones o problemas de su entorno.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Ambiental y de la Salud; Desarrollo Personal y Espiritual",
    defaultDesc:
      "Recrea en sus obras personajes, temas, y tradiciones, valorando la trascendencia a través del tiempo de los artistas y sus obras, por medio de la inclusión de eventos relacionados con el arte y la cultura, en la programación de su tiempo de ocio.",
  },
];
...
const FALLBACK_INDICATORS = [
  ...
  "**Realiza reportes escritos**, gráficos y digitales, de sus investigaciones sobre instituciones, personas, acontecimientos, lugares históricos y eventos relacionados con las artes.",
  ...
  "**Participa en eventos** artísticos, excursiones y visitas guiadas a lugares relacionados con las artes y la cultura (visuales, aplicadas, escénicas y musicales) a nivel local, regional y nacional.",
  "**Valora y defiende** las instituciones, personas, lugares históricos, eventos y acontecimientos relacionados con las artes (visuales, aplicadas, escénicas y musicales) nivel local, regional y nacional."
];

export interface EducacionArtisticaUnidad6toProps {
  user: any;
...
const DRAFT_KEY = "plx:artistica6to_unidad_draft";
...
export default function EducacionArtisticaUnidad6to({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  onCancel,
  onSave,
}: EducacionArtisticaUnidad6toProps) {
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Artística de 6to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-artistica', '6to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-artistica' && cu.grade_id === '6to')
        .map(cu => cu.content);
