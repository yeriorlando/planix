...
// Import compiled 2nd cycle sequences for Matemática 6to
...
const MATEMATICA_6TO_SEQUENCES = Object.values(compiled2ndCycleSequences as Record<string, any>)
  .filter((s: any) => s.subject_id === 'matematica-6to')
  ...
  .map((seq: any, idx: number) => {
    ...
    return {
      ...
      description: seq.description || `Secuencia didáctica de Matemática 6to grado.`,
...
export interface Matematica6toProps {
  user: any;
...
const DRAFT_KEY = "plx:matematica6to_draft";
...
export default function Matematica6to({
  user,
  selectedSequence,
  ...
  const getSequenceData = (idx: number) => {
    const original = MATEMATICA_6TO_SEQUENCES[idx] || MATEMATICA_6TO_SEQUENCES[0];
    ...
    try {
      ...
      if (!latestPlan) {
        latestPlan = {
          id: "temp-plan-id",
          docente_id: user.id,
          titulo: (activeActivityData as any)?.title || (activeActivityData as any)?.activityTitle || activeSequenceData?.title || "Matemática 6to Grado",
