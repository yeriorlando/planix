...
export interface InglesUnidad5toProps {
  user: any;
...
const DRAFT_KEY = "plx:ingles5to_unidad_draft";
...
export default function InglesUnidad5to({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (inglesPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Inglés de 5to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('ingles', '5to');
      const filteredCustom = customUnitsRaw
        .filter(cu => (cu.subject_id === 'ingles' || cu.subject_id === 'ingles-pri') && cu.grade_id === '5to')
        .map(cu => cu.content);
