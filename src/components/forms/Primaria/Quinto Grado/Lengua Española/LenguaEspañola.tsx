...
const LENGUA_5TO_SEQUENCES = [
  ...
  const seqId = `seq-${idx+1}-lengua-5to`;
  ...
  return {
    ...
    description: LENGUA_5TO_DESCRIPTIONS[idx],
...
const DRAFT_KEY = "plx:lengua5to_draft";
...
export default function LenguaEspañola({
  ...
  const getSequenceData = (idx: number) => {
    const original = LENGUA_5TO_SEQUENCES[idx] || LENGUA_5TO_SEQUENCES[0];
    ...
    try {
      ...
      if (!latestPlan) {
        latestPlan = {
          id: "temp-plan-id",
          docente_id: user.id,
          titulo: (activeActivityData as any)?.title || (activeActivityData as any)?.activityTitle || activeSequenceData?.title || "Lengua Española 5to Grado",
