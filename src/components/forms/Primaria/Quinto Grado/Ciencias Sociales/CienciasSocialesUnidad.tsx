...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Selecciona fuentes seguras y confiables, con el propósito de comprender y comunicar informaciones sobre geografía e historia de la isla de Santo Domingo y el continente americano; con la finalidad de respetar la autoría de las informaciones, comprenderlas y comunicarlas",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Crítico y Creativo; Resolución de Problemas; Científica y Tecnológica",
    defaultDesc:
      "Analiza conceptos y situaciones referidos a la geografía, la historia como ciencia, el continente americano, el Caribe y los procesos históricos de República Dominicana; con la finalidad de plantear su punto de vista sobre sus hallazgos de manera lógica, crítica y creativa.",
  },
  {
    key: "etica",
...
const FALLBACK_INDICATORS = [
  ...
  "**Evidencia comprensión de** las situaciones que generaron acontecimientos históricos importantes en el mundo y en la República Dominicana durante los periodos estudiados.",
...
const DRAFT_KEY = "plx:sociales5to_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias Sociales de 5to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('sociales', '5to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'sociales' && cu.grade_id === '5to')
        .map(cu => cu.content);
