...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Comunica la importancia de los cambios que se dan en su cuerpo, sus deberes y derechos a fin de construir relaciones de respeto y equidad en su entorno familiar, escolar y social, tomando en cuenta los derechos consignados sobre la niñez y su valor como hijo e hija de Dios.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Tecnológica y Científica",
    defaultDesc:
      "Analiza la contribución y perjuicios del trabajo humano, las ciencias y las tecnologías en el ámbito social, cultural y espiritual de las personas a fin de mejorar la creación de Dios, con creatividad, sentido ético y responsabilidad.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Desarrollo Personal y Espiritual; Ambiental y de la Salud",
    defaultDesc:
      "Propone acciones que favorecen el cuidado a los demás y al entorno natural como casa común en la familia y la escuela a partir de la vida y las enseñanzas de Jesús de Nazaret para afianzar sus principios morales, su autoestima con respeto, responsabilidad, asertividad y gratitud.",
  },
];
...
const ASIGNATURAS_PRIMARIA = [
  ...
  { name: "Formación Integral Humana y Religiosa", icon: Globe },
...
const DRAFT_KEY = "plx:formacion5to_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          (p.asignatura?.toLowerCase().includes("formacion-humana") || p.titulo?.toLowerCase().includes("formacion-humana")) &&
          (p.grado?.toLowerCase().includes("5") || p.grado?.toLowerCase().includes("quint"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Formación Integral Humana y Religiosa de 5to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('formacion-humana', '5to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'formacion-humana' && cu.grade_id === '5to')
        .map(cu => cu.content);
