...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Ofrece explicaciones científicas y tecnológicas a partir de analizar y evaluar preguntas o hipótesis de observaciones, medición, modelos y experimentación de fenómenos naturales en contexto próximo o experimentado o modelado en ciencias de la vida, físicas, de la tierra y el universo",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Crítico y Creativo; Resolución de Problemas; Científica y Tecnológica",
    defaultDesc:
      "Aplica organizados y sistemáticos procedimientos científicos y tecnológicos, que evalúa mientras explora o experimenta, simula o construye, haciéndose consciente de sus cuestionamientos e inferencia a partir de su observación y medición llevando a cabo vivencias, experimentos, proyectos, exploraciones y observaciones guiadas",
  },
  {
    key: "etica",
...
const FALLBACK_INDICATORS = [
  ...
  "**Analiza, propone y** lleva a cabo medidas de prevención de algunas enfermedades recurrentes en el país producidas por bacterias, virus y protozoarios a fin de mantener la salud individual y colectiva;",
  "**Analiza, argumenta e** implementa colaborativamente medidas sobre el uso sostenible de los recursos naturales como el agua y su ciclo, suelo y minerales, recursos escualores, el cuidado del Medioambiente y la contaminación, los diferentes problemas ambientales ante el calentamiento global, cambio climático, desforestación y pérdida de biodiversidad costera que afectan el desarrollo sostenible en la región y el país.",
...
const DRAFT_KEY = "plx:naturales5to_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          ...
          (p.grado?.toLowerCase().includes("5") || p.grado?.toLowerCase().includes("quinto"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias de la Naturaleza de 5to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('naturales', '5to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'naturales' && cu.grade_id === '5to')
        .map(cu => cu.content);
