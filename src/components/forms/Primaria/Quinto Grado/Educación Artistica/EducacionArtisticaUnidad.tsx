...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Utiliza las posibilidades expresivas de su cuerpo y su voz, así como elementos plásticos, gestuales, sonoros y visuales: con la finalidad de representar ideas, emociones, vivencias propias y de otras personas.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Científica y Tecnológica",
    defaultDesc:
      "Usa crítica y creativamente elementos de la tecnología, la ciencia y la cultura, a fin de integrarlos en sus investigaciones y expresiones artísticas.",
  },
  {
    key: "etica",
    label: "Ética y Ciudadana; Ambiental y de la Salud; Desarrollo Personal y Espiritual",
    defaultDesc:
      "Identifica distintas prácticas presentes en su comunidad, orientadas al cuidado de salud, de la naturaleza y del patrimonio cultural, con la finalidad de integrarlas en sus expresiones artísticas.",
  },
];
...
const FALLBACK_INDICATORS = [
  ...
  "**Difunde sus obras** valorando la importancia de participar, con criterios comunicativos en montajes, representaciones, exposiciones y muestras artísticas.",
  ...
  "**Incorpora elementos tradicionales** de su cultura regional a sus producciones artísticas reconociendo aportes de artistas dominicanos a la cultura nacional."
];
...
const DRAFT_KEY = "plx:artistica5to_unidad_draft";
...
export default function EducacionArtisticaUnidad({
  ...
  const toggleComp = (k: CompetenciaKey) => {
    const labelMap: Record<CompetenciaKey, string> = {
      ...
      logico: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Científica y Tecnológica",
      etica: "Ética y Ciudadana; Ambiental y de la Salud; Desarrollo Personal y Espiritual",
    };
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Artística de 5to. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-artistica', '5to');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-artistica' && cu.grade_id === '5to')
        .map(cu => cu.content);
