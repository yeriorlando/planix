...
const FALLBACK_INDICATORS = [
  ...
  "**Describe y lleva** a cabo prácticas y hábitos saludables de alimentación, cuidado y protección de sus órganos, su salud y y bienestar personal y comunitario.",
...
const DRAFT_KEY = "plx:naturales3ro_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Ciencias de la Naturaleza de 3ro. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('naturales', '3ro');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'naturales' && cu.grade_id === '3ro')
        .map(cu => cu.content);
