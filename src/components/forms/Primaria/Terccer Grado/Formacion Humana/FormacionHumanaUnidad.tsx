...
const FALLBACK_INDICATORS = [
  ...
  "**Relata situaciones de** violencia, abuso, riesgo físico, sexual, psicológico y emocional que hacen daño a los niños y las niñas tanto en la familia como en el ambiente que le rodea.",
...
const DRAFT_KEY = "plx:formacion3ro_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Formación Integral Humana y Religiosa de 3ro. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('formacion-humana', '3ro');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'formacion-humana' && cu.grade_id === '3ro')
        .map(cu => cu.content);
