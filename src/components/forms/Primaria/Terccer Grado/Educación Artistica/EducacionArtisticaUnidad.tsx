...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Utiliza elementos de los lenguajes artísticos, con el fin de comunicar ideas, conceptos y sentimientos.",
  },
  {
    key: "logico",
    label: "Pensamiento Lógico, Creativo y Crítico; Resolución de Problemas; Científica y Tecnológica",
    defaultDesc:
      "Utiliza elementos de los lenguajes artísticos, incorporando aspectos de la ciencia y la tecnología, con la finalidad de aplicar soluciones creativas en sus trabajos.",
  },
  {
    key: "etica",
    label: "Ambiental y de la Salud; Ética y Ciudadana; Desarrollo Personal y Espiritual",
    defaultDesc:
      "Participa en actividades artísticas grupales, mostrando respeto por la diversidad de opiniones y formas de expresiones al emplear el reciclaje, sonidos y movimientos a fin de promover el cuidado de la salud y del medio ambiente.",
  },
];
...
const DRAFT_KEY = "plx:artistica3ro_unidad_draft";
...
export default function EducacionArtisticaUnidad({
  ...
  const toggleComp = (k: CompetenciaKey) => {
    const labelMap: Record<CompetenciaKey, string> = {
      ...
      etica: "Ambiental y de la Salud; Ética y Ciudadana; Desarrollo Personal y Espiritual",
    };
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
      ...
      const socPlans = plannings
        .filter(p => 
          (p.asignatura?.toLowerCase().includes("artistica") || p.titulo?.toLowerCase().includes("artistica") || p.asignatura?.toLowerCase().includes("artística") || p.titulo?.toLowerCase().includes("artística")) &&
          (p.grado?.toLowerCase().includes("3") || p.grado?.toLowerCase().includes("tercer"))
        )
        .sort((a, b) => new Date(b.creado_en).getTime() - new Date(a.creado_en).getTime());

      if (socPlans.length === 0) {
        toast.error("No se encontró ninguna planificación anterior de Educación Artística de 3ro. (Primaria) para esta cuenta.", { id: "ai-retro" });
        return;
      }
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-artistica', '3ro');
      const filteredCustom = customUnitsRaw
        .filter(cu => cu.subject_id === 'educacion-artistica' && cu.grade_id === '3ro')
        .map(cu => cu.content);
