...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Realiza producciones artísticas incorporando aspectos fundamentales de los lenguajes artísticos y del contexto planteado por las ideas y épocas, difundiendo sus creaciones.",
  },
  {
    key: "pensamiento_logico",
    label: "Pensamiento Lógico, Creativo y Crítico",
    defaultDesc:
      "Emprende proyectos artísticos colectivos, valorando críticamente las distintas manifestaciones integradas, al identificar sus componentes y reconocer su trascendencia.",
  },
  {
    key: "resolucion_problemas",
    label: "Resolución de Problemas",
    defaultDesc:
      "Realiza producciones incorporando aspectos fundamentales de los lenguajes artísticos y del contexto planteado por las ideas y épocas, difundiendo sus creaciones.",
  },
  {
    key: "etica_ciudadana",
    label: "Ética y Ciudadana",
    defaultDesc:
      "Emprende proyectos artísticos colectivos que tengan un impacto social o cultural, a partir del análisis del arte popular como fruto de la necesidad de comunicación de los pueblos.",
  },
  {
    key: "cientifica_tecnologica",
    label: "Científica y Tecnológica",
    defaultDesc:
      "Analiza los recursos tecnológicos como protagonistas y mediadores de las tramas, los ambientes y los espacios creados en producciones artísticas.",
  },
  {
    key: "ambiental_salud",
    label: "Ambiental y de la Salud",
    defaultDesc:
      "Promueve el uso adecuado de recursos al crear o valorar en el arte técnicas de reúso, reducción y reciclaje de materiales de desecho y cuidado de los ambientes.",
  },
  {
    key: "desarrollo_personal_espiritual",
    label: "Desarrollo Personal y Espiritual",
    defaultDesc:
      "Incorpora valores en sus interpretaciones y creaciones, utilizando lenguajes artísticos y considerando aspectos de su contexto.",
  },
];
...
const FALLBACK_INDICATORS = [
  "**Realiza foto-reportajes de** acontecimientos de su interés.",
  "**Promueve temas sociales** en sus creaciones, mostrando habilidades técnicas propias de las artes interpretativas y audiovisuales.",
  "**Integra diferentes disciplinas** artísticas en sus creaciones.",
  "**Realiza análisis críticos**, valorando aspectos temáticos y técnicos de las distintas artes.",
  "**Expone características de** manifestaciones artísticas populares.",
  "**Analiza el contenido** e impacto social de distintas expresiones artísticas.",
  "**Integra herramientas y** tecnología adecuada en sus creaciones artísticas individuales y colectivas.",
  "**Musicaliza imágenes visuales**, videos, pequeñas escenas teatrales y eventos escolares.",
  "**Realiza ilustraciones, diseños** y maquetas, al planificar y promover sus producciones artísticas.",
  "**Reconoce elementos identitarios** presentes en sus creaciones y las de otras personas.",
  "**Integra contenido social** a sus obras, a partir del análisis y reflexión de necesidades o problemáticas de su entorno.",
  "**Reconoce el poder** y el impacto del arte en la transmisión de mensajes con ideas de contenido social.",
  "**Utiliza adecuadamente distintas** plataformas virtuales, para promover sus creaciones.",
  "**Argumenta sobre la** importancia de la web como espacio de difusión de las artes de todas partes del mundo.",
  "**Contribuye a la** difusión de eventos artísticos a través de los medios a su disposición.",
  "**Argumenta acerca de** los aportes de las artes, a la salud física y mental de sus intérpretes y la audiencia.",
  "**Escoge adecuadamente el** contenido y mensaje de las obras que produce y aprecia.",
  "**Disfruta la programación** artística como oferta para la cultura del entretenimiento y el ocio productivo.",
  "**Participa colaborativamente en** agrupaciones artísticas escolares y comunitarias.",
  "**Disfruta al ponerse** en contacto con diversas expresiones artísticas comprendiendo algunos de los elementos que las conforman.",
...
export interface EducacionArtisticaUnidadProps {
  user: any;
...
const DRAFT_KEY = "plx:sec_artistica3ro_unidad_draft";
...
export default function EducacionArtisticaUnidad({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  const [compDescs, setCompDescs] = useState<Record<CompetenciaKey, string>>(draft?.compDescs ?? {
    ...
    pensamiento_logico: COMPETENCIAS[1].defaultDesc,
    resolucion_problemas: COMPETENCIAS[2].defaultDesc,
    etica_ciudadana: COMPETENCIAS[3].defaultDesc,
    cientifica_tecnologica: COMPETENCIAS[4].defaultDesc,
    ambiental_salud: COMPETENCIAS[5].defaultDesc,
    desarrollo_personal_espiritual: COMPETENCIAS[6].defaultDesc,
  });
  ...
  const handlePreview = () => {
    ...
    const previewData = {
      ...
      titulo: `Unidad: ${selectedSequence?.name || "Educación Artística"}`,
  ...
  const handleGenerateContent = async () => {
    ...
    try {
      ...
      if (plan) {
        ...
        toast.success("¡Planificación de Educación Artística generada!");
      }
    } catch (err) {
      console.error(err);
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
    ...
    try {
      ...
      if (user?.id) {
        try {
          ...
          const artisticaPlans = plannings
            .filter(p => {
              ...
              const isArtistica = normAsig.includes("artistica") || normTit.includes("artistica") || normAsig.includes("arte") || normTit.includes("arte");
              ...
              return isArtistica && isSecundariaPlan && isGrade3 && !isCurrentPlan;
            })
            .sort((a, b) => new Date(b.creado_en || b.created_at || 0).getTime() - new Date(a.creado_en || a.created_at || 0).getTime());

          if (artisticaPlans.length > 0) {
            latestPlan = artisticaPlans[0];
          }
        } catch (fetchErr) {
      ...
      const planContext = {
        ...
        grado: "3ro Grado de Secundaria",
        level: "SECUNDARIA",
        titulo: latestPlan?.titulo || `${selectedSequence?.name || 'Unidad'} - ${selectedTheme?.name || 'Artes Visuales y Escénicas'}`,
        intencion_pedagogica: latestPlan?.intencion_pedagogica || intencionPedagogica || "Desarrollar y evaluar la apreciación estética y las técnicas artísticas trabajadas en la sesión anterior.",
      };
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-artistica', '3ro Sec');
      const filteredCustom = (Array.isArray(customUnitsRaw) ? customUnitsRaw : [])
        .filter(cu => {
          const sMatch = cu.subject_id === 'educacion-artistica' || cu.subject_id === 'educacion-artistica-sec';
  ...
  return (
    <div className="w-full bg-transparent text-[#1B1B1B] dark:text-zinc-100 font-sans relative overflow-hidden">
      ...
      <div className="mx-auto max-w-5xl px-2 pb-8 pt-14 md:pt-20 text-left relative z-10">
        ...
        >
          ...
          <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            <Field label="Área">
              <input className={inputCls} value="Educación Artística" readOnly />
            </Field>
            <Field label="Unidad">
      ...
      <DailyPlanSelector
        ...
        subjectId={selectedSubject?.id || "educacion-artistica"}
        grade={selectedGrade}
      />
