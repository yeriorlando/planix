...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  {
    ...
    label: "Comunicativa",
    defaultDesc:
      "Muestra dominio motriz en la utilización de su cuerpo para expresar sentimientos, emociones y estados de ánimo, en relación armónica con las demás personas y su entorno social y cultura.",
  },
  {
  ...
  },
  {
    ...
    defaultDesc:
      "Aplica distintos niveles de su capacidad motora con base en sus condiciones físicas naturales para lograr un desempeño motriz eficaz en diversas situaciones de juegos, deportes, trabajo y de la vida cotidiana.",
  },
  {
    key: "etica_ciudadana",
...
const FALLBACK_INDICATORS = [
  "**Pone de manifiesto** sus capacidades físicas: fuerza rápida, velocidad de reacción, traslación y gestual en actividades gimnásticas, recreativas y deportivas.",
  "**Realiza actividades físicas** con eficacia, a partir del mejoramiento de sus capacidades físicas.",
  "**Expone los elementos** históricos más importantes relacionados con el fútbol, el voleibol, el ajedrez, los deportes con raqueta (bádminton, tenis de mesa, tenis) y de la carrera de orientación, a nivel internacional, nacional y local.",
  "**Demuestra a través** del juego, su dominio de los fundamentos básicos en diversas modalidades o adaptaciones de los siguientes deportes: fútbol (fútbol 11 y 5 en grama, futsal y fútbol con cascabeles), voleibol (voleibol, voleibol arena y voleibol sentado), deportes con raqueta, (bádminton, tenis de mesa, tenis) y carrera de orientación.",
  ...
  "**Ejecuta pases a** corta, mediana y larga distancia.",
  "**Recepciona el balón** con diferentes partes del cuerpo y en diferentes situaciones de juego.",
  "**Maneja y domina** el tiro directo, indirecto, toque de cabeza y toque con diferentes partes del cuerpo.",
  "**Interpreta correctamente los** elementos básicos de las reglas del fútbol, el voleibol, el ajedrez, los deportes con raqueta (bádminton, tenis de mesa, tenis) y de la carrera de orientación,",
  "**Domina el pase** de antebrazos y toque de manos altas en las primeras situaciones de juego.",
  "**Domina la acomodada** y ataques en las primeras situaciones de juego.",
  "**Domina con jugadas** a ambos lados de la cancha las recepciones y bloqueo.",
  "**Participa en juegos** de 4 contra 4 y 6 contra 6 en cancha modificada, utilizando el saque de abajo, recepción del saque, acomodada y remate.",
  "**Identifica los tipos** de aperturas (regulares e irregulares) que existen para ataque y defensa.",
  "**Crea esquemas de** aperturas para ataque y defensa.",
  "**Selecciona su repertorio** de aperturas de acuerdo a sus fortalezas y debilidades como ajedrecista.",
  "**Relaciona las habilidades** y destrezas físico-deportivas con el desarrollo de una imagen corporal saludable.",
  "**Reconoce y valora** la importancia de la actividad física para su desarrollo integral y para la ocupación de su tiempo libre.",
  "**Adopta correctamente las** posiciones fundamentales de estos deportes antes de ejecutar los movimientos.",
  "**Ejecuta correctamente movimientos** ofensivos y defensivos de los deportes con raqueta (bádminton, tenis de mesa, tenis), a partir de los elementos básicos del reglamento."
];

export interface EducacionFisicaUnidadProps {
  user: any;
...
const DRAFT_KEY = "plx:sec_educacionfisica3ro_unidad_draft";
...
export default function EducacionFisicaUnidad({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
        ...
        try {
          ...
          const fisicaPlans = plannings
            .filter(p => {
              ...
              const isFisica = normAsig.includes("fisica") || normTit.includes("fisica");
              ...
              return isFisica && isSecundariaPlan && isGrade3 && !isCurrentPlan;
            })
            .sort((a, b) => new Date(b.creado_en || b.created_at || 0).getTime() - new Date(a.creado_en || a.created_at || 0).getTime());

          if (fisicaPlans.length > 0) {
            latestPlan = fisicaPlans[0];
          }
        } catch (fetchErr) {
      ...
      const planContext = {
        ...
        grado: "3ro Grado de Secundaria",
        level: "SECUNDARIA",
        titulo: latestPlan?.titulo || `${selectedSequence?.name || 'Unidad'} - ${selectedTheme?.name || 'Aptitud Física y Deportes'}`,
        intencion_pedagogica: latestPlan?.intencion_pedagogica || intencionPedagogica || "Desarrollar y evaluar las habilidades motrices y capacidades físicas trabajadas en la sesión anterior.",
      };
    ...
    try {
      ...
      const extractUnitContents = (unit: any) => {
        ...
        const cleanAndAdd = (text: string, targetArr: string[]) => {
          ...
          if (!trimmed) return;
          // Check if this string is accidentally stringified JSON
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-fisica', '3ro Sec');
      const filteredCustom = (Array.isArray(customUnitsRaw) ? customUnitsRaw : [])
        .filter(cu => {
          const sMatch = cu.subject_id === 'educacion-fisica' || cu.subject_id === 'educacion-fisica-sec';
