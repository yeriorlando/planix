...
type CompetenciaKey = 
  ...
  | "desarrollo_personal_espiritual"
  | "etica_ciudadana";
...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  ...
  },
  {
    ...
    label: "Científica y Tecnológica",
    defaultDesc:
      "Analiza a partir de criterios éticos los avances científicos y tecnológicos con la finalidad de proteger la vida y la sostenibilidad del entorno natural.",
  },
  {
    key: "pensamiento_logico",
    label: "Pensamiento Lógico, Creativo y Crítico",
    defaultDesc:
      "Analiza las habilidades, actitudes, características, talentos, intereses y valores vinculados al mundo laboral, a fin de ir visualizando su proyecto de vida en la sociedad.",
  },
  {
    key: "ambiental_salud",
    label: "Ambiental y de la Salud",
    defaultDesc:
      "Desarrolla acciones de protección y cuidado al entorno natural, a partir de los valores de Jesús de Nazaret y otros líderes, en su familia, escuela y comunidad, a fin de fomentar el cuidado de la casa común creada por Dios para el disfrute de todos.",
  },
  {
    key: "resolucion_problemas",
    label: "Resolución de Problemas",
    defaultDesc:
      "Fomenta la resolución y prevención de conflictos interpersonales, basado en el diálogo y la escucha, a fin de propiciar la convivencia armónica en su familia, la escuela y la sociedad.",
  },
  {
    key: "desarrollo_personal_espiritual",
    label: "Desarrollo Personal y Espiritual",
    defaultDesc:
      "Responde a las interrogantes que surgen en la etapa de la adolescencia mediante el cultivo de valores humanos, morales, éticos y religiosos, con la finalidad de actuar de forma apropiada en su familia, la escuela y la comunidad.",
  },
  {
    key: "etica_ciudadana",
    label: "Ética y Ciudadana",
    defaultDesc:
...
const ASIGNATURAS_SECUNDARIA = [
  ...
  { name: "Ciencias Sociales", icon: Globe },
  { name: "Ciencias de la Naturaleza", icon: Leaf },
  { name: "Educación Artística", icon: Palette },
  { name: "Educación Física", icon: Dumbbell },
  { name: "Formación H. Integral R.", icon: Heart },
  { name: "Lenguas Extranjeras - Inglés", icon: Languages },
  { name: "Lenguas Extranjeras - Francés", icon: Languages },
];

const FALLBACK_INDICATORS = [
  "**Argumenta sobre el** respeto y la valoración de la vida apoyándose en diferentes textos.",
  "**Argumenta sobre la** necesidad del cumplimiento de los derechos humanos vinculados a la defensa de la vida como don de Dios.",
  "**Muestra aceptación de** sus compañeros/as sin importar color de piel, ritmo de aprendizaje, práctica religiosa, género, entre otras características.",
  "**Argumenta sobre la** importancia de la diversidad vocacional para el servicio a la sociedad.",
  "**Explica el proceso** para decidir respecto a una profesión u oficio.",
  "**Expone sus habilidades**, talentos, actitudes, características, intereses e inclinaciones para el trabajo.",
  "**Argumenta sobre la** necesidad o importancia de la existencia de un clima armónico en las relaciones familiares e interpersonales.",
  "**Promueve el consenso** y la negociación en sus relaciones interpersonales para la resolución de conflictos en los diferentes contextos.",
  "**Identifica problemáticas y** conflictos que se experimentan en las relaciones familiares y sociales y proporciona elementos para un manejo adecuado.",
  "**Muestra en sus** relaciones interpersonales valores de Jesús Nazaret.",
  "**Argumenta sobre la** práctica de valores a partir de las enseñanzas de Jesús.",
  "**Presenta la propuesta** de los valores de Jesús para la puesta en práctica en su familia y comunidad.",
  "**Argumenta sobre los** beneficios y riesgos de los avances científicos y tecnológicos al desarrollo humano y la sostenibilidad de entorno natural.",
  "**Expone las incidencias** de los avances científico-tecnológicos en la persona, la sociedad y el medioambiente.",
  "**Hace propuesta para** el uso adecuado de las tecnologías en sus actividades cotidianas.",
  "**Presenta los valores** que predicó y practicó Jesús sobre el cuidado de la creación y del entorno natural.",
  "**Argumenta, a partir** de textos diversos, sobre los valores de Jesús y otros líderes para una ecología integral.",
  "**Propone soluciones para** contrarrestar la contaminación y el deterioro del planeta, a partir de los valores Jesús.",
  "**Expone las principales** interrogantes que se plantean los seres humanos en la búsqueda del sentido de la vida, a través de la dimensión religiosa.",
  "**Argumenta las respuestas** que dan las grandes religiones a las interrogantes existenciales del ser humano.",
  "**Argumenta a partir** de diferentes textos las respuestas al sentido de la vida que dan las religiones."
];

export interface FormacionHumanaUnidadProps {
  user: any;
...
const DRAFT_KEY = "plx:sec_formacion3ro_unidad_draft";
...
export default function FormacionHumanaUnidad({
  user,
  selectedSequence,
  selectedTheme,
  selectedSubtheme,
  ...
  const [compDescs, setCompDescs] = useState<Record<CompetenciaKey, string>>(draft?.compDescs ?? {
    ...
    cientifica_tecnologica: COMPETENCIAS[1].defaultDesc,
    pensamiento_logico: COMPETENCIAS[2].defaultDesc,
    ambiental_salud: COMPETENCIAS[3].defaultDesc,
    resolucion_problemas: COMPETENCIAS[4].defaultDesc,
    desarrollo_personal_espiritual: COMPETENCIAS[5].defaultDesc,
    etica_ciudadana: COMPETENCIAS[6].defaultDesc,
  });
  ...
  const toggleComp = (k: CompetenciaKey) => {
    const labelMap: Record<CompetenciaKey, string> = {
      ...
      cientifica_tecnologica: "Científica y Tecnológica",
      pensamiento_logico: "Pensamiento Lógico, Creativo y Crítico",
      ambiental_salud: "Ambiental y de la Salud",
      resolucion_problemas: "Resolución de Problemas",
      desarrollo_personal_espiritual: "Desarrollo Personal y Espiritual",
      etica_ciudadana: "Ética y Ciudadana",
    };
  ...
  const handlePreview = () => {
    ...
    const previewData = {
      ...
      titulo: `Unidad: ${selectedSequence?.name || "Formación Integral Humana y Religiosa"}`,
  ...
  const handleGenerateContent = async () => {
    ...
    try {
      ...
      if (plan) {
        ...
        toast.success("¡Planificación de Formación Integral Humana y Religiosa generada!");
      }
    } catch (err) {
      console.error(err);
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    toast.loading("Generando retroalimentación con IA para 3er Grado de Secundaria...", { id: "ai-retro" });
    ...
    try {
      ...
      if (user?.id) {
        try {
          ...
          const formacionPlans = plannings
            .filter(p => {
              const normAsig = (p.asignatura || p.area || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, "");
              const normTit = (p.titulo || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, "");
              const isFormacion = normAsig.includes("formacion") || normTit.includes("formacion") || normAsig.includes("humana") || normTit.includes("humana") || normAsig.includes("religiosa");
              
              const normPlanGrado = (p.grado || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, "");
              const isSecundariaPlan = (p.nivel === "SECUNDARIA" || normPlanGrado.includes("sec") || normPlanGrado.includes("secundaria")) && !normPlanGrado.includes("primaria");
              const isGrade3 = normPlanGrado.includes("3") || normPlanGrado.includes("tercer") || normPlanGrado.includes("3ro");
              ...
              return isFormacion && isSecundariaPlan && isGrade3 && !isCurrentPlan;
            })
            .sort((a, b) => new Date(b.creado_en || b.created_at || 0).getTime() - new Date(a.creado_en || a.created_at || 0).getTime());

          if (formacionPlans.length > 0) {
            latestPlan = formacionPlans[0];
          }
        } catch (fetchErr) {
          console.warn("Could not fetch previous plans:", fetchErr);
        }
      }
      ...
      const planContext = {
        ...
        grado: "3ro Grado de Secundaria",
        level: "SECUNDARIA",
        titulo: latestPlan?.titulo || `${selectedSequence?.name || 'Unidad'} - ${selectedTheme?.name || 'Valores y Espiritualidad'}`,
        intencion_pedagogica: latestPlan?.intencion_pedagogica || intencionPedagogica || "Desarrollar y evaluar la convivencia armónica, valores éticos y crecimiento espiritual trabajados en la sesión anterior.",
      };
      ...
      const result = await generateRetroalimentacion(planContext);
      setRetroalimentacion(cleanAIPedagogicalText(result.retroalimentacion || ""));
      toast.success("¡Retroalimentación generada con éxito para 3er Grado de Secundaria!", { id: "ai-retro" });
    } catch (err) {
      console.error(err);
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const extractUnitContents = (unit: any) => {
        if (!unit) return { conceptual: [], procedural: [], attitudinal: [] };
        ...
        if (unit.conceptual) cleanAndAdd(unit.conceptual, conceptuals);
        if (unit.procedural) cleanAndAdd(unit.procedural, procedurals);
        if (unit.attitudinal) cleanAndAdd(unit.attitudinal, attitudinals);
        if (unit.content_blocks) processItem(unit.content_blocks);

        // 2. Extract from unit.themes and subthemes
        if (Array.isArray(unit.themes)) {
          unit.themes.forEach((t: any) => {
            if (t.conceptual) cleanAndAdd(t.conceptual, conceptuals);
            if (t.procedural) cleanAndAdd(t.procedural, procedurals);
            if (t.attitudinal) cleanAndAdd(t.attitudinal, attitudinals);
            if (Array.isArray(t.subthemes)) {
              t.subthemes.forEach((st: any) => {
                if (st.conceptual) cleanAndAdd(st.conceptual, conceptuals);
                if (st.procedural) cleanAndAdd(st.procedural, procedurals);
                if (st.attitudinal) cleanAndAdd(st.attitudinal, attitudinals);
              });
            }
          });
        }
      ...
      // 3. Fetch custom units from D1/Supabase and merge with static units & selectedSequence
      const customUnitsRaw = await requestD1<any[]>('/api/custom-units').catch(() => []);
      const staticUnits = getUnitsBySubjectAndGrade('formacion-humana', '3ro Sec');
      const filteredCustom = (Array.isArray(customUnitsRaw) ? customUnitsRaw : [])
        .filter(cu => {
          const sMatch = cu.subject_id === 'formacion-humana' || cu.subject_id === 'formacion-humana-sec';
          const gId = (cu.grade_id || '').toLowerCase();
          const gMatch = gId.includes('3ro') || gId === '3ro sec' || gId === 'secundaria-3ro';
          return sMatch && gMatch;
        })
        .map(cu => cu.content || cu);
      ...
      if (selectedSequence && (selectedSequence.id || selectedSequence.name)) {
        map.set(selectedSequence.id || selectedSequence.name, selectedSequence);
      }
      ...
      selectedPlans.forEach(plan => {
        ...
        const planSecName = (plan.secuencia || pData.secuencia || plan.unidad || pData.unidad || '').toLowerCase().trim();
        
        let unit = null;
        if (unitId) {
          unit = globalUnits.find(u => u.id === unitId);
        }
        if (!unit && planSecName) {
          unit = globalUnits.find(u => (u.name?.toLowerCase().trim() === planSecName) || (u.id?.toLowerCase().trim() === planSecName));
        }
        if (!unit && selectedSequence) {
          unit = selectedSequence;
        }
        
        if (unit && !seenUnitIds.has(unit.id || unit.name)) {
          seenUnitIds.add(unit.id || unit.name);
      ...
      // If no plans matched any unit but selectedSequence exists, extract directly from selectedSequence
      if (contentBlocks.length === 0 && selectedSequence) {
        const extracted = extractUnitContents(selectedSequence);
        if (extracted.conceptual.length > 0 || extracted.procedural.length > 0 || extracted.attitudinal.length > 0) {
          contentBlocks.push({
            title: selectedSequence.name || 'Unidad',
            conceptual: extracted.conceptual,
            procedural: extracted.procedural,
            attitudinal: extracted.attitudinal
          });
      ...
      const formatBlockField = (blocks: any[], field: 'conceptual' | 'procedural' | 'attitudinal') => {
        const validBlocks = blocks.filter(b => b[field] && b[field].length > 0);
      ...
      // Helper to clean fallback raw fields
      const cleanFallbackText = (val: any, fieldKey: 'conceptual' | 'procedural' | 'attitudinal'): string => {
        if (!val) return '';
        if (typeof val === 'object') {
          return val[fieldKey] || val.descripcion || '';
        }
        if (typeof val === 'string') {
          ...
          if (trimmed.startsWith('{') && trimmed.endsWith('}')) {
            try {
              const parsed = JSON.parse(trimmed);
              return cleanFallbackText(parsed, fieldKey);
            } catch {}
          }
          return trimmed;
        }
      ...
      if (!finalConceptual) {
        ...
        selectedPlans.forEach((plan) => {
          const pData = plan.customFields || plan.formData || {};
          const val = cleanFallbackText(plan.conceptual || pData.conceptual, 'conceptual');
          if (val) uniqueConceptuals.add(val);
        });
      ...
      if (!finalProcedural) {
        ...
        selectedPlans.forEach((plan) => {
          const pData = plan.customFields || plan.formData || {};
          const val = cleanFallbackText(plan.procedural || pData.procedural || plan.procedimental || pData.procedimental, 'procedural');
          if (val) uniqueProcedurals.add(val);
        });
      ...
      if (!finalAttitudinal) {
        ...
        selectedPlans.forEach((plan) => {
          const pData = plan.customFields || plan.formData || {};
          const val = cleanFallbackText(plan.attitudinal || pData.attitudinal || plan.actitudinal || pData.actitudinal, 'attitudinal');
          if (val) uniqueAttitudinals.add(val);
        });
  ...
  return (
    <div className="w-full bg-transparent text-[#1B1B1B] dark:text-zinc-100 font-sans relative overflow-hidden">
        ...
        >
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label="Grado" required>
              <input className={inputCls} value="3ro. (Secundaria)" readOnly />
            </Field>
          ...
          <div className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            <Field label="Área">
              <input className={inputCls} value="Formación Integral Humana y Religiosa" readOnly />
            </Field>
            <Field label="Unidad">
        ...
        >
          {(() => {
            const compIcons: Record<CompetenciaKey, React.ReactNode> = {
              ...
              cientifica_tecnologica: <Atom className="h-4 w-4" />,
              pensamiento_logico: <BrainCircuit className="h-4 w-4" />,
              ambiental_salud: <Leaf className="h-4 w-4" />,
              resolucion_problemas: <Target className="h-4 w-4" />,
              desarrollo_personal_espiritual: <Brain className="h-4 w-4" />,
              etica_ciudadana: <Scale className="h-4 w-4" />,
            };
            return (
                ...
                {showAreasDropdown && (
                  <>
                    ...
                    <div className="absolute left-0 right-0 mt-1.5 p-3 bg-white dark:bg-zinc-900 border border-neutral-200 dark:border-zinc-800 rounded-lg shadow-xl z-20 max-h-60 overflow-y-auto space-y-1.5 animate-in fade-in slide-in-from-top-2 duration-150">
                      {ASIGNATURAS_SECUNDARIA.map((subject) => {
              ...
              return (
                <div className="mt-3 flex flex-wrap gap-2">
                  {areasArticuladas.map((name, idx) => {
                    const subject = ASIGNATURAS_SECUNDARIA.find(s => s.name === name);
      ...
      <DailyPlanSelector
        ...
        subjectId={selectedSubject?.id || "formacion-humana"}
        grade={selectedGrade}
      />
