...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  ...
  },
  {
    ...
    defaultDesc:
      "Domina su acción motriz, a partir de su nivel de percepción y apreciación de patrones, tamaños, formas, direcciones , relaciones espaciales y temporales dinámicas en diversos entornos, con base en la noción de su esquema corporal.",
  },
  {
    key: "resolucion_problemas",
    label: "Resolución de Problemas",
    defaultDesc:
      "Utiliza el desarrollo de distintos niveles de desempeño motriz, a partir de sus condiciones físicas naturales, a fin de alcanzar eficacia motora progresiva en situaciones variables (juegos, deportes, trabajo).",
  },
  {
    key: "etica_ciudadana",
  ...
  },
  {
    ...
    defaultDesc:
      "Descubre mediante análisis las características de implementos deportivos específicos, con la finalidad de usarlos en forma efectiva en situaciones de juegos y deportes.",
  },
  {
    key: "ambiental_salud",
...
const FALLBACK_INDICATORS = [
  ...
  "**Expone los elementos** históricos relevantes del atletismo, el béisbol, el softbol y las artes marciales y de modalidades adaptadas y paralímpicas de esas disciplinas en el plano internacional, nacional y local.",
  "**Ejecuta correctamente las** técnicas básicas en las diferentes pruebas atléticas (caminatas, carreras, saltos y lanzamientos).",
  "**Fildea batazos de** rolling y de flay en el infielder y outfielder.",
  "**Ejecuta correctamente la** mecánica de bateo (agarre del bate, postura y ejecución del swing), ante lanzamientos con diferentes localizaciones, durante situaciones de juego.",
  "**Corre las bases** atendiendo a la situación del juego (outs, compañeros en base, marcador, entre otros).",
  "**Aplica las habilidades** motrices necesarias para la ejecución de las técnicas básicas en las diferentes pruebas atléticas (caminata, salidas altas y agachadas, carreras hasta 60 metros y carreras hasta 600 metros, salto largo, modalidad natural y colgante, salto alto modalidad tijera).",
  "**Reconoce y aplica** los elementos básicos del reglamento del atletismo, el béisbol, el softbol y las artes marciales y de modalidades adaptadas y paralímpicas de esas disciplinas",
  "**Ejecuta las acciones** defensivas de los infielders y outfielders (fildeo y tiro).",
  "**Ejecuta la mecánica** de lanzador de frente y de lado.",
  "**Analiza partidas identificando** las estrategias que intervienen en situaciones críticas (para defensa o ataque).",
  "**Juega partidas con** anotación, identificando las estrategias que intervienen.",
  "**Realiza tiros con** precisión a distancias largas y cortas y en recta a distancia reducida entre el box y el home plate.",
  "**Realiza técnicas defensivas** y ofensivas en situaciones de riesgo de su integridad física.",
  "**Ejecuta los fundamentos** básicos de las artes marciales (posiciones, desplazamientos, bloqueos, patadas, puño, agarres y proyecciones, caídas, defensas, combates) y de modalidades adaptadas de esas disciplinas",
  "**Construye e instala** casas de campaña y refugios, convencionales y con recursos del medio.",
  "**Se orienta utilizando** la brújulas convencionales o elaboradas, a partir de recursos no convencionales, según puntos cardinales y sistema de referencia (GPS, mapas, otros).",
  "**Aplica técnicas de** prevención de riesgos y primeros auxilios ante situaciones de emergencia e imprevistos.",
...
const DRAFT_KEY = "plx:sec_educacionfisica2do_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
        ...
        try {
          ...
          const fisicaPlans = plannings
            .filter(p => {
              ...
              return isFisica && isSecundariaPlan && isGrade2 && !isCurrentPlan;
            })
      ...
      const planContext = {
        ...
        grado: "2do Grado de Secundaria",
        level: "SECUNDARIA",
        titulo: latestPlan?.titulo || `${selectedSequence?.name || 'Unidad'} - ${tituloPlan || 'Capacidades Físicas'}`,
        intencion_pedagogica: latestPlan?.intencion_pedagogica || intencionPedagogica || "Desarrollar y evaluar las habilidades motrices y capacidades físicas de la unidad.",
      };
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-fisica', '2do Sec');
      const filteredCustom = customUnitsRaw
        .filter(cu => (cu.subject_id === 'educacion-fisica' || cu.subject_id === 'educacion-fisica-sec') && (cu.grade_id === '2do Sec' || cu.grade_id === 'secundaria-2do'))
        .map(cu => cu.content);
