...
const COMPETENCIAS: { key: CompetenciaKey; label: string; defaultDesc: string }[] = [
  ...
  },
  {
    ...
    defaultDesc:
      "Constata la importancia de aplicar criterios éticos en el uso de las ciencias y las tecnologías, con el fin de mejorar la calidad de vida de las personas y del entorno natural.",
  },
  {
  ...
  },
  {
    ...
    defaultDesc:
      "Identifica acciones a favor del cuidado de la vida en todas sus manifestaciones, a fin de garantizar su salud personal y colectiva desde los diferentes contextos.",
  },
  {
    key: "resolucion_problemas",
    label: "Resolución de Problemas",
    defaultDesc:
      "Constata las principales dificultades que viven los adolescentes en su desarrollo y en sus relaciones sociales y familiares, para buscar alternativas de solución a los conflictos que se les presenten.",
  },
  {
  ...
  },
  {
    ...
    defaultDesc:
      "Descubre en los evangelios la novedad de la vida y los valores practicados por Jesús de Nazaret, con la finalidad de compararlos con los asumidos en su familia, escuela y la comunidad e integrarlos en su relación con los demás.",
  },
];
...
const FALLBACK_INDICATORS = [
  "**Argumenta sobre el** cuidado y respeto a la vida en todas sus manifestaciones a partir de los derechos humanos, valores morales, culturales y religiosos.",
  "**Explica la defensa** de la dignidad de la persona y las diferentes manifestaciones culturales y religiosas de su familia y su comunidad, mediante textos bíblicos y otros documentos.",
  "**Identifica la diversidad** religiosa presente en su familia y su entorno y expresa de forma natural su experiencia religiosa.",
  "**Explica sobre el** esfuerzo y dedicación que requiere el trabajo para el bienestar de las personas, la familia y la sociedad.",
  "**Argumenta sobre el** aporte del trabajo de las mujeres en el desarrollo de su familia y de la sociedad.",
  "**Argumenta sobre la** importancia de la diversidad vocacional y profesional para mejorar la calidad de vida de las personas y el desarrollo social.",
  "**Identifica sus emociones,** sentimientos, acciones y comportamientos relacionados a su persona, familia y comunidad y los expresa a través de valores humanos y cristianos.",
  "**Distingue situaciones que** favorecen o desfavorecen la convivencia armónica en su medio familiar, escolar y comunitario y busca apoyo para su solución.",
  "**Utiliza el diálogo** como medio de solución a los conflictos que se le presentan y promueve la paz en sus relaciones interpersonales.",
  "**Argumenta sobre el** estilo de vida basado en el amor que Jesús de Nazaret, propone a las personas y lo relaciona con hechos y situaciones de la vida diaria.",
  "**Analiza episodios relacionados** con la vida, pasión, muerte y resurrección de Jesús de Nazaret y los compara con actitudes y acciones en su vida personal, familiar, escolar y social.",
  "**Distingue la presencia** de valores que promueven el bien común, desde el ejemplo de Jesús de Nazaret, en sí mismo, sus amigos y familiares.",
  "**Expone las incidencias** de los avances científicos y tecnológicos en la persona, la sociedad y el entorno natural.",
  "**Argumenta sobre el** uso ético de las nuevas tecnologías en sus actividades cotidianas e identifica los beneficios que la tecnología aporta en diferentes situaciones de la vida diaria.",
  "**Examina los daños** y riesgos que conlleva el uso inadecuado de los inventos tecnológicos para la persona y el entorno natural.",
  "**Promueve acciones de** cuidado y respeto a la vida en todas sus manifestaciones y al entorno natural a nivel personal, familiar y comunitario.",
  "**Argumenta sobre lo** establecido en las normas y leyes de nuestro país en torno a la protección de la vida y el cuidado del entorno natural.",
  "**Realiza acciones para** cuidar su salud personal y colectiva en las actividades familiares, escolares y con sus amigos.",
  "**Explica los cambios** físicos, sexuales, emocionales y espirituales y sus consecuencias en el desarrollo de la adolescencia.",
  "**Comunica sus preocupaciones** e interrogantes personales, emocionales, sociales y espirituales sobre la etapa que vive y da respuestas a sus miedos e inseguridades.",
  "**Asume la responsabilidad** en relación con su cuidado personal, físico y emocional en su familia, escuela y comunidad para afianzar su conocimiento personal y espiritual."
];
...
const DRAFT_KEY = "plx:sec_formacion1ro_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    toast.loading("Generando retroalimentación con IA para 1er Grado de Secundaria...", { id: "ai-retro" });
    ...
    try {
        ...
        try {
          ...
          const formacionPlans = plannings
            .filter(p => {
              ...
              const isFormacion = normAsig.includes("formacion") || normAsig.includes("humana") || normAsig.includes("religiosa") || normTit.includes("formacion") || normTit.includes("humana");
              ...
              const isGrade1 = normPlanGrado.includes("1") || normPlanGrado.includes("primer") || normPlanGrado.includes("1ro");
              
              const currentTitle = `${selectedSequence?.name || "Unidad"} - ${tituloPlan || ""}`;
              const isCurrentPlan = p.titulo === currentTitle && p.customFields?.fecha === fecha;
                
              return isFormacion && isSecundariaPlan && isGrade1 && !isCurrentPlan;
            })
      ...
      const planContext = latestPlan || {
        ...
        grado: "1er Grado de Secundaria",
        level: "SECUNDARIA",
        titulo: `${selectedSequence?.name || 'Unidad'} - ${tituloPlan || 'Valores y Desarrollo Personal'}`,
        intencion_pedagogica: intencionPedagogica || "Reflexionar sobre los valores éticos, morales y espirituales de la unidad.",
      };
      ...
      toast.success("¡Retroalimentación generada con éxito para 1er Grado de Secundaria!", { id: "ai-retro" });
    } catch (err) {
      console.error(err);
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('formacion-humana', '1ro Sec');
      const filteredCustom = customUnitsRaw
        .filter(cu => (cu.subject_id === 'formacion-humana' || cu.subject_id === 'formacion-humana-sec') && (cu.grade_id === '1ro Sec' || cu.grade_id === 'secundaria-1ro'))
        .map(cu => cu.content);
      ...
      selectedPlans.forEach(plan => {
        ...
        if (!unit && planSecName) {
          unit = globalUnits.find(u => u.name?.toLowerCase().trim() === planSecName);
        }
        if (!unit && selectedSequence) {
          unit = globalUnits.find(u => u.id === selectedSequence.id || u.name?.toLowerCase().trim() === selectedSequence.name?.toLowerCase().trim());
        }
        
        if (unit && !seenUnitIds.has(unit.id)) {
        ...
        >
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Field label="Grado" required>
              <input className={inputCls} value="1ro. (Secundaria)" readOnly />
            </Field>
