...
const FALLBACK_INDICATORS = [
  "**Improvisa espontáneamente movimientos** corporales individuales y en grupos, comunicando y expresando ideas, sentimientos, emociones y circunstancias, con y sin implementos.",
  "**Crea composiciones cortas** en diferentes planos, niveles y direcciones (distribución consciente en el espacio) comunicando y expresando ideas, sentimientos, emociones y circunstancias, con y sin implementos.",
  ...
  "**Ejecuta los elementos** técnicos de los implementos seleccionados, tomando en cuenta los diferentes tipos de agarres.",
  "**Colabora con sus** compañeros/as en la solución de problemáticas técnico-tácticas relacionadas con situaciones de juego en baloncesto, ajedrez y balonmano.",
  "**Realiza calentamiento general** y específico antes de las actividades físicas reconociendo su importancia para la prevención de lesiones.",
  ...
  "**Expone los elementos** históricos más importantes de la gimnasia artística, del baloncesto (convencional, 3 x 3 y/o adaptado), del ajedrez, del balonmano (convencional y/o adaptado) en el plano internacional, nacional y local.",
  "**Aplica los fundamentos** técnicos y reglas básicas del baloncesto, baloncesto adaptado, baloncesto 3 x 3, del balonmano y del balonmano adaptado ante defensa y en situaciones motrices diversas que implican la toma de decisión.",
  "**Identifica semejanzas y** diferencias entre los diferentes tipos de pases, dribles y tiros.",
  "**Crea variantes de** las acciones técnicas del baloncesto, baloncesto adaptado, del balonmano, aprendidas o realizadas en clase.",
  "**Resuelve problemas de** ajedrez a 2 ,1 y 3 jugadas, identificando las tácticas que intervienen.",
  "**Analiza partidas de** ajedrez identificando las tácticas que intervienen en situaciones críticas.",
  "**Juega partidas utilizando** las tácticas básicas en situaciones críticas (para defensa o ataque).",
  "**Indaga, recopila y** socializa juegos tradicionales, populares, colaborativos, competitivos y dinámicas de grupo.",
  "**Participa de manera** activa en la ejecución de juegos tradicionales, populares, colaborativos, competitivos y dinámicas de grupo.",
  "**Interpreta adecuadamente el** valor de los elementos básicos y las reglas generales de código de puntuación de la gimnasia general y la gimnasia artística.",
...
const DRAFT_KEY = "plx:sec_educacionfisica1ro_unidad_draft";
...
<p>${result.ampliacion?.descripcion || "Actividad avanzada de investigación o creación autónoma."}</p>`;
  ...
  const handleGenerateRetroalimentacion = async () => {
    ...
    try {
        ...
        try {
          ...
          const fisicaPlans = plannings
            .filter(p => {
              ...
              return isFisica && isSecundariaPlan && isGrade1 && !isCurrentPlan;
            })
      ...
      const planContext = latestPlan || {
        ...
        grado: "1er Grado de Secundaria",
        level: "SECUNDARIA",
        titulo: `${selectedSequence?.name || 'Unidad'} - ${tituloPlan || 'Capacidades Físicas'}`,
        intencion_pedagogica: intencionPedagogica || "Desarrollar y evaluar las habilidades motrices y capacidades físicas de la unidad.",
      };
  ...
  const handleSynthesize = async (selectedPlans: any[]) => {
    ...
    try {
      ...
      const staticUnits = getUnitsBySubjectAndGrade('educacion-fisica', '1ro Sec');
      const filteredCustom = customUnitsRaw
        .filter(cu => (cu.subject_id === 'educacion-fisica' || cu.subject_id === 'educacion-fisica-sec') && (cu.grade_id === '1ro Sec' || cu.grade_id === 'secundaria-1ro'))
        .map(cu => cu.content);
