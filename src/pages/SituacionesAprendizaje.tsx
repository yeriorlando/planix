...
const ESTRATEGIAS = [
    { value: 'abp', label: 'Aprendizaje Basado en Problemas (ABP)' },
    { value: 'proyecto', label: 'Aprendizaje Basado en Proyectos' },
    { value: 'indagacion', label: 'Indagación Dialógica' },
    { value: 'estudio-caso', label: 'Estudio de Caso' }
];
...
export default function SituacionesAprendizaje() {
    ...
    // Configuración
    const [strategy, setStrategy] = useState('abp');
    ...
    const handleGenerate = async (e: React.FormEvent) => {
        ...
        if (!topic.trim()) { 
            setError('Debes ingresar el tema o unidad.'); 
            return; 
        }
        ...
        const strategyLabel = ESTRATEGIAS.find(s => s.value === strategy)?.label.split('(')[0].trim() || strategy;

        try {
            const data = await generateSituation({
                nivel, grado, asignatura, strategy: strategyLabel, topic, contextInfo
            });

            if (!data) {
                throw new Error('Error al generar la situación de aprendizaje. Intenta de nuevo.');
            }
    ...
    const handlePrint = () => {
        ...
        document.title = `Planix - Situacion de Aprendizaje - ${cleanTopic} - ${cleanAsignatura} - ${cleanGrado}`;
        window.print();
        setTimeout(() => {
    ...
    return (
        <div className="min-h-screen pb-24 print:bg-white print:p-0 w-full flex-1">
            ...
            <main className="max-w-4xl mx-auto px-4 print:max-w-none print:px-0">
                {/* Título Banner */}
                <div className="print:hidden mb-5 bg-gradient-to-r from-indigo-500/10 via-purple-500/5 to-indigo-600/10 dark:from-indigo-500/15 dark:to-purple-600/15 border border-indigo-500/15 dark:border-indigo-500/25 rounded-xl py-3 px-5 flex flex-col md:flex-row items-center gap-3.5 shadow-2xs relative overflow-hidden w-full">
                    ...
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-indigo-500/20 dark:bg-indigo-500/30 flex items-center justify-center shrink-0 border border-indigo-500/30 dark:border-indigo-500/40 relative">
                        <Globe className="w-5 h-5 md:w-6 h-6 text-indigo-650 dark:text-indigo-400 stroke-[2.5]" />
                    </div>
                    ...
                    <div className="text-center md:text-left flex-1 relative z-10">
                        ...
                        <p className="text-slate-655 dark:text-zinc-400 font-medium text-[11px] md:text-xs mt-0.5 max-w-3xl leading-normal">
                            Redacta situaciones de aprendizaje realistas, significativas y alineadas con la Adecuación Curricular. Plantea escenarios con conflicto cognitivo, definición de roles, entregables y competencias.
                        </p>
                    </div>
                </div>
                ...
                <div className="print:hidden space-y-6 mb-12">
                    ...
                    <section className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-3xl p-6 shadow-2xs focus-within:shadow-xs transition-shadow">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-505 dark:text-zinc-400">Nivel</label>
                            ...
                            {/* Grado */}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-505 dark:text-zinc-400">Grado</label>
                                <div className="relative select-none">
                                    <button 
                                        type="button"
                                    ...
                                    {showGradoDropdown && (
                                        <>
                                            ...
                                            <div className="absolute left-0 top-full mt-1 w-full bg-white dark:bg-zinc-955 rounded-lg border border-black/5 dark:border-zinc-800 shadow-lg p-1 z-50 animate-in fade-in slide-in-from-top-1 duration-150 max-h-60 overflow-y-auto">
                                                <div className="space-y-0.5">
                                                    {allowedGrados.map((g) => (
                                                        <button
                                                            key={g}
                            ...
                            {/* Asignatura */}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-505 dark:text-zinc-400">Asignatura</label>
                                <div className="relative select-none">
                                    <button 
                                        type="button"
                                    ...
                                    {showAsignaturaDropdown && (
                                        <>
                                            ...
                                            <div className="absolute left-0 top-full mt-1 w-full bg-white dark:bg-zinc-905 rounded-lg border border-black/5 dark:border-zinc-800 shadow-lg p-1 z-50 animate-in fade-in slide-in-from-top-1 duration-150 max-h-60 overflow-y-auto">
                                                <div className="space-y-0.5">
                    ...
                    {/* PASO 2: Configuración de la Situación */}
                    <section className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-3xl p-6 shadow-2xs focus-within:shadow-xs transition-shadow">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h2 className="text-lg font-black text-slate-800 dark:text-white">Detalles de la Narrativa</h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            {/* Estrategia */}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-555 dark:text-zinc-400">Estrategia Pedagógica</label>
                                <div className="relative select-none">
                                    <button 
                                        type="button"
                                        ...
                                        className="w-full h-10 px-3.5 bg-neutral-50 dark:bg-zinc-900/50 border border-neutral-200 dark:border-zinc-800 rounded-lg text-sm text-[#1B1B1B] dark:text-neutral-100 flex items-center justify-between cursor-pointer hover:bg-slate-50 dark:hover:bg-zinc-850 focus:border-brand-primary outline-none transition-all shadow-xs"
                                    >
                                        <span>{ESTRATEGIAS.find(s => s.value === strategy)?.label || "Seleccionar..."}</span>
                                        <ChevronDown className={`h-4 w-4 text-slate-400 dark:text-zinc-550 transition-transform duration-200 ${showStrategyDropdown ? "rotate-180" : ""}`} />
                                    </button>
                                    
                                    {showStrategyDropdown && (
                                        <>
                                            <div className="fixed inset-0 z-40" onClick={() => setShowStrategyDropdown(false)} />
                                            <div className="absolute left-0 top-full mt-1 w-full bg-white dark:bg-zinc-905 rounded-lg border border-black/5 dark:border-zinc-800 shadow-lg p-1 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                                                <div className="space-y-0.5">
                                                    {ESTRATEGIAS.map((s) => (
                                                        <button
                                                            key={s.value}
                                                        ...
                                                        >
                                                            <span>{s.label}</span>
                                                            {strategy === s.value && <Check className="w-3.5 h-3.5 shrink-0 text-[#1B1B1B] dark:text-white" />}
                                                        </button>
                                                    ))}
                                                </div>
                                            </div>
                            ...
                            {/* Tema */}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-550 dark:text-zinc-400">Tema o Unidad de Aprendizaje</label>
                                <input 
                                    type="text" 
                                    value={topic} 
                        ...
                        {/* Contexto escolar */}
                        <div className="space-y-2">
                            <label className="text-xs font-bold text-slate-550 dark:text-zinc-400 flex items-center gap-1">
                                Contexto Escolar o Intereses de los Estudiantes
                                <span className="text-[10px] text-slate-400 font-medium">(Opcional)</span>
                            </label>
                            <textarea 
                                value={contextInfo} 
                                onChange={e => setContextInfo(e.target.value)} 
                                placeholder="Ej: Escuela ubicada cerca de una cañada con acumulación de basura, estudiantes que muestran gran interés en el béisbol, escuela rural agrícola..." 
                                className="w-full h-24 p-3.5 bg-neutral-50 dark:bg-zinc-900/50 border border-neutral-200 dark:border-zinc-800 rounded-lg text-sm text-[#1B1B1B] dark:text-neutral-100 placeholder:text-neutral-400 focus:border-[#1B1B1B] dark:focus:border-neutral-205 focus:ring-1 focus:ring-[#1B1B1B]/10 outline-none transition-all shadow-xs resize-none" 
                            />
                        </div>
                    </section>
                    ...
                    <div className="flex justify-center pt-6">
                        ...
                        >
                            ...
                            {isGenerating ? 'Redactando Situación IA...' : 'Generar Situación'}
                        </button>
                    </div>
                </div>
                        ...
                        <div className="print:m-0 print:border-none print:shadow-none bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl p-6 md:p-12 shadow-xs relative">
                            ...
                            <div className="print:m-0 print:p-0 max-w-4xl mx-auto space-y-12 print:space-y-6 pb-16 print:pb-4 print:max-w-none print:w-full print:px-0">
                                <div className="text-center space-y-4 print:space-y-2 border-b-2 border-slate-855 dark:border-zinc-750 pb-8 print:pb-4 page-break-inside-avoid">
                                    ...
                                    <div className="inline-block mt-4 bg-indigo-100 dark:bg-indigo-955/40 border border-indigo-300 dark:border-indigo-900/50 text-indigo-850 dark:text-indigo-400 px-6 py-1.5 rounded-lg font-black uppercase tracking-widest text-xs shadow-xs print:border-none print:bg-white print:text-black">
                                        Estrategia: {ESTRATEGIAS.find(s => s.value === strategy)?.label.split('(')[0].trim()}
                                    </div>
                                </div>
