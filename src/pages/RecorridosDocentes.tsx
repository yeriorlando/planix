...
const MOMENTOS = [
    { value: 'inicio', label: 'Inicio (Recuperación de saberes)' },
    { value: 'desarrollo', label: 'Desarrollo (Trabajo activo)' },
    { value: 'cierre', label: 'Cierre (Metacognición)' }
];
...
export default function RecorridosDocentes() {
    ...
    // Configuración
    const [moment, setMoment] = useState('desarrollo');
    ...
    const handleGenerate = async (e: React.FormEvent) => {
        ...
        if (!topic.trim()) { 
            setError('Debes ingresar un tema o actividad.'); 
            return; 
        }
        ...
        try {
            const data = await generateTeacherScript({
                nivel, grado, asignatura, moment, topic
            });

            if (!data) {
                throw new Error('Error al generar el recorrido docente. Intenta de nuevo.');
            }
    ...
    const handlePrint = () => {
        ...
        document.title = `Planix - Recorrido Docente - ${cleanTopic} - ${cleanAsignatura} - ${cleanGrado}`;
        window.print();
        setTimeout(() => {
            ...
            <main className="max-w-4xl mx-auto px-4 print:max-w-none print:px-0">
                ...
                <div className="print:hidden mb-5 bg-gradient-to-r from-emerald-500/10 via-teal-500/5 to-emerald-600/10 dark:from-emerald-500/15 dark:to-teal-600/15 border border-emerald-500/15 dark:border-emerald-500/25 rounded-xl py-3 px-5 flex flex-col md:flex-row items-center gap-3.5 shadow-2xs relative overflow-hidden w-full">
                    ...
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-emerald-500/20 dark:bg-emerald-500/30 flex items-center justify-center shrink-0 border border-emerald-500/30 dark:border-emerald-500/40 relative">
                        <Map className="w-5 h-5 md:w-6 h-6 text-emerald-600 dark:text-emerald-400 stroke-[2.5]" />
                    </div>
                    ...
                    <div className="text-center md:text-left flex-1 relative z-10">
                        ...
                        <p className="text-slate-655 dark:text-zinc-400 font-medium text-[11px] md:text-xs mt-0.5 max-w-3xl leading-normal">
                            Genera guiones de acompañamiento en el aula con preguntas clave para andamiaje de estudiantes rezagados, preguntas de profundización para avanzados y detección de errores comunes.
                        </p>
                    </div>
                </div>
                ...
                <div className="print:hidden space-y-6 mb-12">
                    ...
                    {/* PASO 2: Configuración del Guion */}
                    <section className="bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 rounded-3xl p-6 shadow-2xs focus-within:shadow-xs transition-shadow">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <h2 className="text-lg font-black text-slate-800 dark:text-white">Detalles de la Clase</h2>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {/* Momento */}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-500 dark:text-zinc-400">Momento Pedagógico</label>
                                <div className="flex flex-col sm:flex-row gap-2">
                                    {MOMENTOS.map(m => {
                                        const isSelected = moment === m.value;
                                        return (
                                            <button
                                                key={m.value}
                                                type="button"
                                                onClick={() => setMoment(m.value)}
                                                className={`flex-1 px-4 py-2.5 rounded-xl text-xs font-black transition-all cursor-pointer border ${
                                                    isSelected 
                                                        ? 'bg-brand-primary text-white border-brand-primary shadow-md shadow-brand-primary/20' 
                                                        : 'bg-slate-50 dark:bg-zinc-950 text-slate-550 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-850 border-slate-200 dark:border-zinc-800'
                                                }`}
                                            >
                            ...
                            {/* Tema */}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-slate-500 dark:text-zinc-400">Tema o Actividad</label>
                                <input 
                                    type="text" 
                                    value={topic} 
                                    onChange={e => setTopic(e.target.value)} 
                                    placeholder="Ej: Suma de fracciones, El cuento..." 
                    ...
                    <div className="flex justify-center pt-6">
                        ...
                        >
                            ...
                            {isGenerating ? 'Generando Recorrido IA...' : 'Generar Recorrido'}
                        </button>
                    </div>
                </div>
                            ...
                            <div className="max-w-4xl mx-auto space-y-12 print:space-y-6 pb-16 print:pb-4 print:max-w-none print:w-full print:px-0">
                                <div className="text-center space-y-4 print:space-y-2 border-b-2 border-slate-855 dark:border-zinc-750 pb-8 print:pb-4 page-break-inside-avoid">
                                    <h2 className="text-2xl print:text-xl font-black text-slate-900 dark:text-white uppercase tracking-wide">
                                        RECORRIDO DOCENTE / GUION DE INTERVENCIÓN
                                    </h2>
                                    <h3 className="text-lg print:text-base font-bold text-slate-655 dark:text-zinc-400">
                                        {asignatura.toUpperCase()} - {grado} ({nivel})
                                    </h3>
                                    <div className="inline-block mt-4 bg-emerald-100 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-900/50 text-emerald-800 dark:text-emerald-400 px-6 py-1.5 rounded-lg font-black uppercase tracking-widest text-xs shadow-xs print:border-none print:bg-white print:text-black">
                                        Momento: {MOMENTOS.find(m => m.value === moment)?.label.split('(')[0].trim()}
                                    </div>
                                </div>
