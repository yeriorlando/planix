  ...
  return (
    ...
    >
      <div 
        ...
        className="bg-white dark:bg-zinc-900 rounded-[2.5rem] w-full max-w-[460px] p-8 shadow-2xl relative border border-slate-100 dark:border-zinc-800 cursor-default animate-in fade-in zoom-in-95 duration-200"
      >
        <button 
          type="button"
          onClick={onClose} 
          className="absolute right-6 top-6 w-8 h-8 bg-red-500 hover:bg-red-600 text-white rounded-full flex items-center justify-center transition-colors cursor-pointer shadow-md"
          title="Cerrar"
        >
          <X className="w-4 h-4" strokeWidth={2.5} />
        </button>
        
        <h2 className="text-2xl font-black text-slate-800 dark:text-white mb-6">Nuevo Evento</h2>
        
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
           <div>
              <label className="text-[11px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 block">TÍTULO DEL EVENTO</label>
              <input 
                type="text" 
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder="ej. Reunión de Padres / Examen Parcial" 
                className="w-full h-10 bg-slate-50 dark:bg-zinc-955 border border-slate-200 dark:border-zinc-800 rounded-lg px-4 text-sm font-semibold text-slate-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 focus:border-brand-primary outline-none transition-all shadow-2xs" 
              />
           </div>
           
           <div className="flex gap-4">
              <div className="flex-1">
                 <label className="text-[11px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 block">FECHA</label>
                 <DatePicker 
                   value={date}
                   onChange={setDate}
                   direction="down"
              ...
              <div className="flex-1">
                 <label className="text-[11px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 block">HORA</label>
                 <input 
                   type="time" 
                   value={time}
                   onChange={(e) => setTime(e.target.value)}
                   className="w-full h-10 bg-slate-50 dark:bg-zinc-955 border border-slate-200 dark:border-zinc-800 rounded-lg px-4 text-sm font-semibold text-slate-800 dark:text-white focus:bg-white dark:focus:bg-zinc-900 focus:border-brand-primary outline-none transition-all shadow-2xs cursor-pointer" 
                 />
              </div>
           </div>
           ...
           <div className="flex gap-4">
              <div className="flex-1">
                 <label className="text-[11px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 block">TIPO DE EVENTO</label>
                 <div className="relative select-none">
                     <button 
                         type="button"
                         onClick={() => setShowTypeDropdown(!showTypeDropdown)}
                         className="w-full h-10 px-3.5 bg-slate-50 dark:bg-zinc-955 border border-slate-200 dark:border-zinc-800 rounded-lg text-sm text-[#1B1B1B] dark:text-neutral-100 flex items-center justify-between cursor-pointer hover:bg-slate-100 dark:hover:bg-zinc-800 focus:border-brand-primary outline-none transition-all shadow-xs"
                     >
                         <span>{type}</span>
                         <ChevronDown className={`h-4 w-4 text-slate-400 dark:text-zinc-550 transition-transform duration-200 ${showTypeDropdown ? "rotate-180" : ""}`} />
                     </button>
                             ...
                             <div className="absolute left-0 top-full mt-1 w-full bg-white dark:bg-zinc-900 rounded-lg border border-black/5 dark:border-zinc-800 shadow-lg p-1 z-[120] animate-in fade-in slide-in-from-top-1 duration-150 max-h-60 overflow-y-auto">
                                 <div className="space-y-0.5">
                                     {[
                                         ...
                                         >
                                             <span>{opt}</span>
                                             {type === opt && <Check className="w-3.5 h-3.5 shrink-0 text-[#1B1B1B] dark:text-white" />}
                                         </button>
                                     ))}
                                 </div>
                             </div>
              ...
              <div className="flex-1">
                 <label className="text-[11px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 block">DURACIÓN</label>
                 <div className="relative select-none">
                     <button 
                         type="button"
                     ...
                     >
                         ...
                         <ChevronDown className={`h-4 w-4 text-slate-400 dark:text-zinc-550 transition-transform duration-200 ${showDurationDropdown ? "rotate-180" : ""}`} />
                     </button>
                             ...
                             <div className="absolute left-0 top-full mt-1 w-full bg-white dark:bg-zinc-900 rounded-lg border border-black/5 dark:border-zinc-800 shadow-lg p-1 z-[120] animate-in fade-in slide-in-from-top-1 duration-150 max-h-60 overflow-y-auto">
                                 <div className="space-y-0.5">
                                     {[
                                         ...
                                         >
                                             <span>{opt.label}</span>
                                             {duration === opt.value && <Check className="w-3.5 h-3.5 shrink-0 text-[#1B1B1B] dark:text-white" />}
                                         </button>
                                     ))}
                                 </div>
                             </div>
           ...
           <div>
              <label className="text-[11px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-1.5 block">COLOR ETIQUETA</label>
              <div className="flex gap-3 flex-wrap">
                {[
                  ...
                  <button
                    ...
                    className={`w-9 h-9 rounded-full cursor-pointer transition-all ${col.class} ${color === col.class ? 'ring-4 ring-brand-primary/30 scale-110 border-2 border-white dark:border-zinc-900 shadow-md' : 'border border-slate-200 dark:border-zinc-800 hover:scale-105'}`}
                    title={col.name}
                  />
                ))}
              </div>
           ...
           <button 
             type="submit" 
             className="w-full h-11 bg-[#1B1B1B] dark:bg-white text-white dark:text-zinc-950 rounded-xl text-sm font-black mt-3 hover:scale-[1.02] hover:shadow-lg transition-all cursor-pointer"
           >
              Programar Evento
           </button>
        </form>
