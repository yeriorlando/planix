...
async function run() {
  ...
  for await (const line of rl) {
    ...
    // Check if the step content contains the view_file header for AdminCurriculum.tsx
    if (step.source === 'SYSTEM' && step.content && 
      ...
      if (match) {
        ...
        // Only keep the first occurrence of each line range (which is from the initial view_file calls)
