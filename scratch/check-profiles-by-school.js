...
async function run() {
  try {
    const res = await fetch(`${supabaseUrl}/rest/v1/profiles?school_name=ilike.*EULALIO%20EMILIO%20MANCEBO*`, {
      headers: {
    ...
    if (res.ok) {
      ...
      console.log("Profiles matching school name:", JSON.stringify(data, null, 2));
    } else {
