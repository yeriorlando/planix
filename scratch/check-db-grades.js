...
async function run() {
  try {
    ...
    console.log(`Found ${grades.length} grades in Supabase for classroom ${classId}:`);
    const counts = {};
    grades.forEach(g => {
      counts[g.subject_id] = (counts[g.subject_id] || 0) + 1;
    });
    console.log("Counts per subject ID in Supabase:", counts);
  } catch (err) {
    console.error(err);
  }
}
